...
import com.dassault_systemes.enovia.configuration.modeler.ConfigurationGovernanceCommon;
...
public class emxProductConfigurationBase_mxJPO extends emxDomainObject_mxJPO
{
  ...
  */
    ...
    throws Exception
  {
    ...
    objectSelects.addElement(ConfigurationConstants.SELECT_ATTRIBUTE_RECOMPUTE_LISTPRICE);
    objectSelects.addElement(ConfigurationConstants.SELECT_ATTRIBUTE_LISTPRICE);
...
public MapList getPreciseBOMForPC(Context context, String[] args)throws Exception
 {
		...
		if (ConfigurationConstants.RANGE_VALUE_TRUE
			...
			for (int i = 0; i < logicalFeatureList.size(); i++) {
				MapList evaluatedParts = new MapList();//will get Part from getActiveGBOMStructure with PC filter passed to it
				...
				//For each LF- get active GBOM based on PC Filter binary
				...
				evaluatedParts = lfbean.getActiveGBOMStructure(context, "",
						relSelect, objselect, ConfigurationUtil
								.getBasicRelSelects(context), false, true, 1, 0,
						null, null, DomainObject.FILTER_STR_AND_ITEM,
						strFilterCompiledForm);
                //iterate all parts and populate map to pass as program result
				if (evaluatedParts.size() > 0) {
					...
					for (int k = 0; k < evaluatedParts.size(); k++) {
						...
						infoMap
								.put(
										ConfigurationConstants.SELECT_INTERFACE_EBOM,
										(String) partMap
												.get(ConfigurationConstants.SELECT_INTERFACE_EBOM));
						infoMap
								.put(
										ConfigurationConstants.RELATIONSHIP_TYPE,
										(String) partMap
						...
						infoMap.put(ConfigurationConstants.SELECT_TYPE,
								(String) partMap
...
//        }
    ...
    public Vector getQuantity(Context context, String []args) throws Exception{
        ...
        if(lstObjectIdsList.size() == 1 &&  null != ((HashMap )lstObjectIdsList.get(0)).get(ConfigurationConstants.SELECT_TYPE))
        {
        	String type = ((HashMap )lstObjectIdsList.get(0)).get(ConfigurationConstants.SELECT_TYPE).toString();
        	if(ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION).contains(type)){
        		 vecAttrVals.addElement("");
        		 return vecAttrVals ;
        	}
        	if(null != ((HashMap )lstObjectIdsList.get(0)).get("Root Node")){
        		if(((HashMap )lstObjectIdsList.get(0)).get("Root Node").toString().equals("true")){
        		 vecAttrVals.addElement("");
           		 return vecAttrVals ;
        		}
        	}
        ...
        String[] ObjectIdsList = new String[lstObjectIdsList.size()]; 
        for(int i = 0 ; i < lstObjectIdsList.size() ; i++){
            ...
            ObjectIdsList[i] = (strObjId);
            
        }
        StringList SelectQuantityfromObject = new StringList();
        SelectQuantityfromObject.add("to["+ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES+"].tomid["+
        		ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS+"|from.id=="+strProductConfigurationId+"].attribute["+
        		ConfigurationConstants.ATTRIBUTE_QUANTITY+"].value");
        SelectQuantityfromObject.add("to["+ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES+"].tomid["+
        		ConfigurationConstants.RELATIONSHIP_SELECTED_OPTIONS+"|from.id=="+strProductConfigurationId+"].attribute["+
        		ConfigurationConstants.ATTRIBUTE_QUANTITY+"].value");
        MapList mapListObjectQuantity = DomainObject.getInfo(context, ObjectIdsList, SelectQuantityfromObject,false);
        Iterator<HashMap> itrQuantity = mapListObjectQuantity.iterator();
        while(itrQuantity.hasNext()){
        	Map QuantityMap = itrQuantity.next();
        	if(QuantityMap.size() == 1){
        		Iterator itrQ = QuantityMap.keySet().iterator();
        		vecAttrVals.addElement(QuantityMap.get(itrQ.next().toString()).toString());
        	}else{
        		vecAttrVals.addElement("");
        	}
       }
...
*/
    ...
    public Vector getKeyInValue(Context context, String []args) throws Exception{
        ...
        for (int i = 0 ;i <lstObjectIdsList.size() ;i++ )
        {
          	...
          	String strRelId   = ConfigurationConstants.EMPTY_STRING;
          	String sKeyInType = ConfigurationConstants.EMPTY_STRING; 
            if (lstObjectIdsList.get(i) instanceof  HashMap ) {
                HashMap map = (HashMap )lstObjectIdsList.get(i) ;
                strObjId = (String)map.get(DomainConstants.SELECT_ID);
                strRelId = (String)map.get(DomainConstants.SELECT_RELATIONSHIP_ID);
                sKeyInType = (String)map.get(ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE);
            }else if (lstObjectIdsList.get(i) instanceof  Hashtable ) {
                Hashtable htObj = (Hashtable)lstObjectIdsList.get(i) ;
                strObjId = (String)htObj.get(DomainConstants.SELECT_ID);
                strRelId = (String)htObj.get(DomainConstants.SELECT_RELATIONSHIP_ID);
                sKeyInType = (String)htObj.get(ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE);
            }
            
            if(ProductLineCommon.isNotNull(sKeyInType) && sKeyInType.equals("Blank")){
            	vecAttrVals.addElement("");
            }else{
              DomainObject domFtr = new DomainObject(strObjId);
              String strDomFtrType = domFtr.getInfo(context,DomainConstants.SELECT_TYPE);
            	if(ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION).contains(strDomFtrType))
            {
                vecAttrVals.addElement("");
            }
            else
            {
                StringBuffer selectable = new StringBuffer();
              	...
              	String strKeyInValue = MqlUtil.mqlCommand(context , mqlCmd, true,strRelId,selectable.toString(),ConfigurationConstants.DELIMITER_PIPE);
                if(ConfigurationConstants.RANGE_VALUE_DATE.equals(sKeyInType)){ 
					try
					{
							Date keyInDate =  new Date(Long.parseLong(strKeyInValue));
							int iDateFormat 		= eMatrixDateFormat.getEMatrixDisplayDateFormat();
							DateFormat dFormat 		= DateFormat.getDateInstance(iDateFormat/*DateFormat.FULL*/, context.getLocale());
							strKeyInValue	    = dFormat.format(keyInDate);
					}
					catch(Exception ep){}
                               
				}
                vecAttrVals.addElement(strKeyInValue);
            	}
            }
        }
         return vecAttrVals ;
     ...
     * Selectable used by CFG Team to get Display Name for Selected Options for PC in COnfiguration Filter
     * @param context- the eMatrix <code>Context</code> object
    ...
    public String getProductConfigurationFeatures(Context context,String[] args){
		...
		try{
			...
			if (ProductLineCommon.isNotNull(prodcutConfigurationId)){
				...
				selectables.add("from."+ConfigurationConstants.SELECT_TYPE);
				...
				StringList   childOfCFs            = ProductLineUtil.getChildTypesIncludingSelf(context, ConfigurationConstants.TYPE_CONFIGURATION_FEATURE);
				for(int i=0;i<featureListMap.size();i++){
					Map temp = (Map)featureListMap.get(i);
					String strPrntType = (String) temp.get("from."+ConfigurationConstants.SELECT_TYPE);
					if(childOfCFs.contains(strPrntType)) {
						String strPrntName = (String) temp.get("from."+ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME);
						String strChldName =(String) temp.get("to."+ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME);
						features.append(strPrntName);
						features.append(":");
						features.append(strChldName);
    ...
    public boolean showCommandForPCCreate(Context context,String args[]) throws Exception
    {
        ...
        if(strContextId!=null && !strContextId.isEmpty())
        {
            ...
            String strObjectType = domCtxObj.getInfo(context, DomainObject.SELECT_TYPE);
	   	  ...
	   	  if(ProductLineConstants.STATE_INACTIVE.equals(strObjectState) || ProductLineConstants.STATE_OBSOLETE.equals(strObjectState))
	   	     {
                showCommand = false;
	   	     }else
	   	     {
      ...
      public StringList getMPRIncludeOIDs(Context context, String[] args) throws Exception
 {
		...
		lstMultiValueList.add(strSelectOptions);
		...
		MapList mlMprResultIds = DomainObject.getInfo(context, (String[]) ids
				.toArray(new String[ids.size()]), objSelects, lstMultiValueList);
	    ...
	    public MapList getProductConfigurationForGrid(Context context, String[] args)throws FrameworkException {
	    	...
	    	try{
	    		...
	    		String [] idArr = selectIds.split(",");
	  		    //-----------------------------------		  
	  		    // all PCs ID from PC summary passed 
	  		    //-----------------------------------	
	    		if(selectIds.trim().isEmpty()){
	    			selectIds = (String)paramList.get("AllPCID");
	    			idArr = selectIds.split(",");
	    		}
	    		//Get info on all PC at a time
	    		if (ProductLineCommon.isNotNull(selectIds)){
	    			MapList mlPCMAPLIST=DomainObject.getInfo(context, idArr, strListSelectstmts);
	    			for (int count=0; count < mlPCMAPLIST.size(); count++ ){
	    				Map newMap = new HashMap() ;
	    				newMap=(Map)mlPCMAPLIST.get(count);
	    				colList.add(newMap);
	    			}
	    		}
/*	    		if (ProductLineCommon.isNotNull(selectIds)){
	    ...
	    public MapList getCFStructureForPCCompareGrid(Context context, String[] args)throws FrameworkException {
	    	...
	    	try{
	    		...
	    		StringBuilder typeKindCF = new StringBuilder(50);
	    		typeKindCF.append("type.kindof[");
	    		typeKindCF.append(ConfigurationConstants.TYPE_CONFIGURATION_FEATURE);
	    		typeKindCF.append("]");
	    		...
	    		String selectIds = (String) requestMap.get("selectId");
	    		StringList slPCIds = FrameworkUtil.splitString(selectIds,",");
	  		    //-----------------------------------		  
	  		    // all PCs ID from PC summary passed 
	  		    //-----------------------------------	
	    		if(selectIds.trim().isEmpty()){
	    			selectIds = (String)requestMap.get("AllPCID");
	    			slPCIds = FrameworkUtil.splitString(selectIds,",");
	    		}
	    		...
	    		slObjSelects.addElement(typeKindCF.toString());
	    		...
	    		slRelSelects.addElement("physicalid[connection]");
	    		...
	    		StringList slFilterBinaries = new StringList();
	    		//Modeler API call to get conflicting options CO rel physical IDs
	    		Map mpConflictingOptionsIDs = com.dassault_systemes.enovia.configuration.modeler.ProductConfiguration.getConflictOptionsForPCs(context, slPCIds, slFilterBinaries);
	    		...
	    		for (int i = 0; i < mapCFStructure.size(); i++) {
	    			...
	    			Map mapConflictingOptions = new HashMap(); // To Show/hide Conflict icon for each CO on compare page.
    				String strRelId = (String) tempMap.get("physicalid[connection]");
    				
    				Map mapConfigSelectionStatus = new HashMap(); //to exclude all those options for which the attribute[Configuration Selection Status].value is not Chosen/Default/Required.
	    			if (tempMap.get(cellIdSelect) instanceof StringList){
						...
						for (int j = 0;j<listSelect.size(); j++) {
							...
							String strConfigSelectionStatus = (String) slAttrConfigSelectionStatus.get(j);
	    					mapConfigSelectionStatus.put(strobject, strConfigSelectionStatus);
							boolean hasAnyConflictingOption = mpConflictingOptionsIDs.containsKey(strobject);
							if(hasAnyConflictingOption){
								StringList slConflictingOptions = (StringList) mpConflictingOptionsIDs.get(strobject);
			    				if(ProductLineCommon.isNotNull(strRelId)&&slConflictingOptions.contains(strRelId)){
			    					isConflicting = true;
			    					mapConflictingOptions.put(strobject, isConflicting);
			    				}else{
			    					isConflicting = false;
			    					mapConflictingOptions.put(strobject, isConflicting);
			    				    }
						...
						boolean hasAnyConflictingOption = mpConflictingOptionsIDs.containsKey(cellId);
						if(hasAnyConflictingOption){
							StringList slConflictingOptions = (StringList) mpConflictingOptionsIDs.get(cellId);
		    				if(ProductLineCommon.isNotNull(strRelId)&&slConflictingOptions.contains(strRelId)){
		    					isConflicting = true;
		    					mapConflictingOptions.put(cellId, isConflicting);
		    				}else{
		    					isConflicting = false;
		    					mapConflictingOptions.put(cellId, isConflicting);
		    				}
					...
					String strTypeKindCF = (String) tempMap.get(typeKindCF.toString());
	    			...
	    			if(strTypeKindCF.equalsIgnoreCase("true") && (subFeatureCount >0 || isTopLevelFeature || isListOfNull(tempMap.get(cellIdKeyInValue)))){
	    				aOptionCount = 0;
	    				...
	    				if(optionValues!=null){
	    					if(optionValues instanceof String){
	    						childOptionList.add(optionValues.toString());
	    					}else{
	    					...
	    					if (tempMap.get(cellIdKeyInValue) instanceof StringList){
	    						...
	    						for (int j = 0;j<listSelect.size(); j++) {
	    							...
	    							if(ProductLineCommon.isNotNull(cellKeyToProcess)){
	    								processCellIdrowMap(context,KeyInValueMap,strobject,cellKeyToProcess, true,false,exportToExcel,false);
	    							}
	    						}
	    					}else{
	    						...
	    						if(ProductLineCommon.isNotNull(cellKeyToProcess)){
	    							String cellId = (String)tempMap.get(cellIdSelect);
	    							processCellIdrowMap(context,KeyInValueMap,cellId,cellKeyToProcess, true,false,exportToExcel,false);
	    						}
	    			...
	    			if(tempMap.get(DomainConstants.SELECT_ID) != null){
	    				if(currentRowId.equals(fromId)){
	    					if(tempMap.get(cellIdSelect) !=null){
	    						putCellValues (context,tempMap, cellIdvaluesMap,mapConflictingOptions,exportToExcel,mapConfigSelectionStatus);
	    					}
	    					aOptionCount++;
	    				}else {
		 ...
		 */
	    private void processCellIdrowMap(Context context,Map cellIdvaluesMap, String cellId, String strName, boolean keyIn, boolean isCF, boolean exportToExcel,boolean isConflicting)throws FrameworkException {
	    	try {
	    		String str ="";
	    		String strFirstTag = "";
	    		String exportInKeyIn = "";
	    		String conflictingIcon ="";
	    		String line = "<hr size=\"1\" style=\"margin-top:3px; margin-bottom: 3px;\" />";
	    		String blankSpace = "<p style=\"visibility:hidden;\">_</p>";
	    		String languageStr = context.getSession().getLanguage();
	    		String shortHandNotationForKeyIn =EnoviaResourceBundle.getProperty(context, SUITE_KEY,
	    				"emxConfiguration.Table.SubHeader.SelectedOptionsComparison_ShortNotation",languageStr);
	    		String titleForConflictIcon = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxConfiguration.Title.ConflictIcon",languageStr);
	    		String exportToExcelConflictIcon = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxConfiguration.ExportToExcel.ConflictIcon",languageStr);
	    		...
	    		if(isConflicting){
	    			if(exportToExcel){
	    				conflictingIcon = exportToExcelConflictIcon;
	    			}else{
	    				conflictingIcon = "<img src=\"images/iconStatusComplaintUrgent.png\" title=\""+ titleForConflictIcon +"\" border=\"0\" height=\"16\"></img>";	
	    			}
	    		}
	    		...
	    		if (cellIdvaluesMap.containsKey(cellId)){
	    			String strcellval =  "";
	    			if(exportToExcel){
	    				strcellval = ((String)cellIdvaluesMap.get(cellId)).concat("\n").concat(strName).concat(exportInKeyIn).concat(conflictingIcon);
	    			}else{
	    				strcellval = ((String)cellIdvaluesMap.get(cellId)).replace("</table>", "<tr><td style=\" vertical-align:middle;\">")+line+XSSUtil.encodeForHTML(context,strName)+" "+conflictingIcon+"</td>"+str+"</tr></table>";
	    			}
	    			cellIdvaluesMap.put(cellId,strcellval);
	    		}else{
	    			if(exportToExcel){
	    				cellIdvaluesMap.put(cellId,strName+exportInKeyIn+conflictingIcon);
	    			}else{
    					cellIdvaluesMap.put(cellId,"<table width=\"100%\" ><tr><td width=\"100%\" >"+XSSUtil.encodeForHTML(context,strName)+" "+conflictingIcon+"</td>"+strFirstTag+"</tr></table>");
	    				}
	    			}
	    	} catch (Exception e) {
	    		throw new   FrameworkException(e.getMessage());
	     ...
	     * */
	    private void putCellValues (Context context,Map tempMap, Map cellIdvaluesMap,Map mapConflictingOptions,boolean exportToExcel,Map mapConfigSelectionStatus) throws FrameworkException{
	    	String cellIdSelect ="tomid["+ProductLineConstants.RELATIONSHIP_SELECTED_OPTIONS+ "].from.id";
	    	...
	    	try {
				if (tempMap.get(cellIdSelect) instanceof StringList){
					...
					for (int j = 0;j<listSelect.size(); j++) {
						...
						boolean isConflicting = false;
						String strobject = (String) listSelect.get(j);
						isConflicting = (boolean) mapConflictingOptions.get(strobject);
						...
						processCellIdrowMap(context,cellIdvaluesMap,strobject,(String)tempMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME), false,isCF,exportToExcel,isConflicting);
					}
					}
				}else{
					boolean isCF =  false;
					boolean isConflicting = false;
					String cellId = (String)tempMap.get(cellIdSelect);
					isConflicting = (boolean) mapConflictingOptions.get(cellId);
					if(((String)tempMap.get(ConfigurationConstants.SELECT_TYPE)).equals(ConfigurationConstants.TYPE_CONFIGURATION_FEATURE)){
						isCF = true;
					}
					...
					if(CONFIGURATION_SELECTION_STATUS_CHOSEN.equalsIgnoreCase(strConfigSelectStatus)
							...
							|| CONFIGURATION_SELECTION_STATUS_REQUIRED.equalsIgnoreCase(strConfigSelectStatus)){
						System.out.println("Attribute Configuration Selection Status to be Processed = "+strConfigSelectStatus);
					processCellIdrowMap(context,cellIdvaluesMap,cellId,(String)tempMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_DISPLAY_NAME), false,isCF,exportToExcel,isConflicting);
					}
	    	...
	    	if(cellIdvaluesMap.containsKey("ActualRowId")){
	    		for(int rCount = 0; rCount < returmrowList.size(); rCount++)
	    		{
	    			...
	    			if(internalMap.get(UITableGrid.KEY_ROW_ID)!=null)
	    			{
	    				...
	    				{
	    					StringList listKeyCellId = new StringList();
	    					StringList listKeyCellValue = new StringList();
	    					if (internalMap.get(UITableGrid.KEY_CELL_ID)!=null && internalMap.get(UITableGrid.KEY_CELL_VALUE)!=null)
	    					{
	    						listKeyCellId = (StringList)internalMap.get(UITableGrid.KEY_CELL_ID);
	    						listKeyCellValue = (StringList)internalMap.get(UITableGrid.KEY_CELL_VALUE);
	    						...
	    						if(cellIdvaluesMap.get(cellIdSelect) != null){
	    							...
	    							{
	    								tempList.add(cellIdvaluesMap.get(cellIdSelect).toString());
	    							}else{
        ...
        throws Exception{
	    	...
	    	if (objectType == null || !mxType.isOfParentType(context,objectType,ConfigurationConstants.TYPE_PRODUCT_CONFIGURATION)) {
	    		//productconfigurationStructList = (MapList) ${CLASS:emxPLCCommon}.getStructureListForType(context, args);
	    		productconfigurationStructList = (MapList) JPO.invoke(context, "emxPLCCommon", null, "getStructureListForType", args, MapList.class);
	    	}
 ...
 */
	...
	public static List getRuleCondition(Context context, String[] args)
		...
		MapList mapRuleList = new MapList();
		StringList operatorList = new StringList();
		List leftExpList = new StringList();
		List rightExpList = new StringList();
		StringList finalExpList = new StringList();
		try {
			HashMap programMap = (HashMap) JPO.unpackArgs(args);
			Map ruleIdsList = (Map) programMap.get("paramList");
			StringList stconflictRules = (StringList) ruleIdsList
	...
	public List getContext(Context context, String[] args)
			throws Exception {
		StringList contexList = new StringList();
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		Map ruleIdsList = (Map) programMap.get("paramList");
		...
		if(strType!=null && !strType.equalsIgnoreCase("")){
			...
			strTypeIcon = "iconLogicalFeature22.png";
		}
		}
		StringBuffer sbBuffer = new StringBuffer();
 ...
 */
    ...
    public int updateSelectedOptionsFilter(Context context,String[] args) throws Exception
    {
		...
		String refineSelMode = ConfigurationConstants.RANGE_VALUE_FALSE;
		try{
		   refineSelMode  = 	(String)EnoviaResourceBundle.getProperty(context,"emxConfiguration.Configurator.RefineSelectionMode");
 	   	}catch(Exception ex){
		...
		if (strPCFilterBinaryUpdate != null && !strPCFilterBinaryUpdate.equals("")){
 	       ...
 	       try
 	       {
    	       ...
    	       if(!isPCFrozen){
				if(ConfigurationConstants.RANGE_VALUE_TRUE.equalsIgnoreCase(refineSelMode)){
				 	System.out.println("In JPO - refineSelMode :" + refineSelMode);
					pcInstance.setFilterBinary(context,strPCId);
    ...
    */
	...
	public StringList getProductContextForProductConfiguration(Context context,String[] args) throws Exception
	{
		...
		for(Iterator iterator = objectList.iterator(); iterator.hasNext();)
		{
			...
			if(strType.equalsIgnoreCase(ConfigurationConstants.TYPE_LOGICAL_FEATURE) && ProductLineCommon.isNotNull(strProductName))
			{
				...
				String strTypeIcon = "../common/images/iconSmallProduct.png";
	...
	/**This method return the List Price on PC and Dirty PC ICON if PC is "Out Of Date".
	 * Which will be display on Product Configuration Properties Page in Product Context only.
	 * This is field program.
	 * @param  context
	 * @param  args
	 * @return List Price on PC + Dirty PC ICON (if PC is Out Of Date)
	 * @throws Exception
	 * @since  R420
	 */
	
	public String getPCListPriceForPropertiesPage(Context context, String[] args) throws Exception
	{
	 try
	   {
		...
		String  objectId     = (String)  requestMap.get("objectId");
		com.dassault_systemes.enovia.configuration.modeler.ProductConfiguration PCConfig = new com.dassault_systemes.enovia.configuration.modeler.ProductConfiguration(objectId);
		boolean      isListPriceUpToDate    = PCConfig.isListPriceUpToDate(context);
		double       pcListPrice            = PCConfig.getListPrice(context);
		boolean      isPCFrozen             = ConfigurationGovernanceCommon.isFrozenState(context,objectId);
		StringBuffer outOfDatePCStatusICON  = new StringBuffer();
		String       strOutOfDate           = EnoviaResourceBundle.getProperty(context, "Configuration","emxProduct.Title.OutOfDate",context.getSession().getLanguage());
		String       strOutOfDateMobileMode = EnoviaResourceBundle.getProperty(context, "Configuration","emxProduct.Title.OutOfDateMobileMode",context.getSession().getLanguage());
		String       strOutOfDateEditMode   = EnoviaResourceBundle.getProperty(context, "Configuration","emxProduct.Title.OutOfDateEditMode",context.getSession().getLanguage());
		
		if(!isListPriceUpToDate){
			if(isMobileMode){
				outOfDatePCStatusICON.append("&nbsp;<b style='color:red'>"+XSSUtil.encodeForXML(context, strOutOfDateMobileMode)+"</b>");
			} else if(reportFormat != null){
				outOfDatePCStatusICON.append(" "+strOutOfDateMobileMode);
			} else if("true".equals(PFmode) || "edit".equals(strMode) || isPCFrozen){
				outOfDatePCStatusICON.append("&nbsp;<img src=\"../common/images/iconStatusAlert.gif\" border=\"0\" title=\""+strOutOfDateEditMode+"\" />");
			}
			else{
				outOfDatePCStatusICON.append("&nbsp;<a href=\"JavaScript:emxFormLinkClick('../configuration/ProductConfigurationRecomputeListPrice.jsp?mode=RecalculateOnProperties&");
				outOfDatePCStatusICON.append("objectId="+XSSUtil.encodeForXML(context, objectId)+"','content', '', '','')\">");
				outOfDatePCStatusICON.append("<img src=\"../common/images/iconStatusAlert.gif\" border=\"0\" title=\""+XSSUtil.encodeForXML(context, strOutOfDate)+"\" />");
				outOfDatePCStatusICON.append("</a>");
			}
		}
		
		return pcListPrice+""+outOfDatePCStatusICON;
	   }
	   catch(Exception exp)
	   {
	...
	/**This method return the StringList of List Price on PC and Dirty PC ICON if PC is "Out Of Date".
	 * Which will be display on Product Configuration Summary Page in Product Context only.
	 * This is column program.
	 * @param  context
	 * @param  args
	 * @return StringList of List Price on PC + Dirty PC ICON (if PC is Out Of Date)
	 * @throws Exception
	 * @since  R420
	 */
	public StringList getPCListPriceForSummaryPage(Context context, String[] args) throws Exception
	{
	 try
	   {	
		boolean    isMobileMode           = UINavigatorUtil.isMobile(context);
		StringList listPriceForPCs        = new StringList();
		...
		{
			Hashtable    pcDetailsMap          = (Hashtable)objectList.get(i);
			String       strPCId               = (String) pcDetailsMap.get(DomainObject.SELECT_ID);
			boolean      recomputeListPrice    = Boolean.parseBoolean((String)pcDetailsMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_RECOMPUTE_LISTPRICE));
			String       pcListPrice           = (String) pcDetailsMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_LISTPRICE);	
			boolean      isPCFrozen            = ConfigurationGovernanceCommon.isFrozenState(context,strPCId);
			StringBuffer outOfDatePCStatusICON = new StringBuffer();
			
			if(recomputeListPrice){
				if(isMobileMode){
					outOfDatePCStatusICON.append("<b style='color:red'>"+XSSUtil.encodeForXML(context, strOutOfDateMobileMode)+"</b>");
				} else if(reportFormat != null){
					outOfDatePCStatusICON.append(strOutOfDateMobileMode);
				} else if(isPCFrozen){
					outOfDatePCStatusICON.append("<img src=\"../common/images/iconStatusAlert.gif\" border=\"0\" title=\""+XSSUtil.encodeForXML(context, strOutOfDateEditMode)+"\" />");
				} else{
					outOfDatePCStatusICON.append("<a href=\"JavaScript:emxTableColumnLinkClick('../configuration/ProductConfigurationRecomputeListPrice.jsp?mode=RecalculateOnSummaryColumn&amp;");
					outOfDatePCStatusICON.append("objectId="+XSSUtil.encodeForXML(context, strPCId)+"','', '', 'true','listHidden')\">");
					outOfDatePCStatusICON.append("<img src=\"../common/images/iconStatusAlert.gif\" border=\"0\" title=\""+XSSUtil.encodeForXML(context, strOutOfDate)+"\"/>");
					outOfDatePCStatusICON.append("</a>");
				}
			} 
			
			listPriceForPCs.add(pcListPrice+" "+outOfDatePCStatusICON);
		}
		return listPriceForPCs;
	   }
	   catch(Exception exp)
	   {
		   throw new FrameworkException(exp.getMessage());
	...
	/**This method is used to check Product Configuration Summary Page is in Product Context or not.
	 * If Product Configuration Summary Page is in Product Context then it will return true,
	 * else false.
	 * This is Access Function
	 * @param  context
	 * @param  args
	 * @return true/false
	 * @throws Exception
	...
	public boolean isProductContextPCSummary(Context context, String[] args) throws Exception
	{
	 try
	   {
		boolean      isProductContextPCSummary = false;
		HashMap      programMap                = (HashMap) JPO.unpackArgs(args);
		String       objectId                  = (String) programMap.get("objectId");
		...
		if(childOfProduct.contains(strType) && !childOfProductVariant.contains(strType))
		{
			isProductContextPCSummary = true;
		}
		
		return isProductContextPCSummary;
	   }
	   catch(Exception exp)
	   {
    ...
    /**
     * Access check for commands based on MQL Expression DesignRuleActivation value, state and Validation Status attribute value of Product Configuration
     * @param context the eMatrix <code>Context</code> object
     * @param args holds the Object Id of the Product Configuration.
     * @return boolean true if access is allowed, and false otherwise.
     * @throws Exception if the operation fails
     * @since R421
     */
    public boolean showValidateDesignRuleUX(Context context, String args[]) throws Exception
    {
      //Getting programMap from the args
      HashMap programMap = (HashMap) JPO.unpackArgs(args);
      String objectId = (String)programMap.get("objectId");
      boolean bDisplay = false;
      //get MQL Expression DesignRuleActivation value
      //boolean showDesignRuleUX = (boolean) ${CLASS:emxPLCCommon}.showDesignRuleUX(context, args);
      boolean showDesignRuleUX = (boolean) JPO.invoke(context, "emxPLCCommon", null, "showDesignRuleUX", args, Boolean.class);
      if(showDesignRuleUX){
    	  boolean bSalable = isSalableProductConfiguration(context, objectId);
          if(!bSalable)
          {
    ...
    /**
     * return a list of Feature Option Name pair for Product Configuration
     *
     * @param context- the eMatrix <code>Context</code> object
     * @param args- holds the HashMap containing the following arguments
     * @return  String- Marketing Features
     ...
     * @since  R421
     */
	public String getPCFeatureOptions(Context context,String[] args) throws Exception{
		...
		try{
			...
			if (ProductLineCommon.isNotNull(prodcutConfigurationId)){
				SelectList selectables = new SelectList(3);
				selectables.add("from."+ConfigurationConstants.SELECT_NAME);
				selectables.add("to."+ConfigurationConstants.SELECT_NAME);
				selectables.add("from."+ConfigurationConstants.SELECT_TYPE);
				featureListMap=ProductConfiguration.getSelectedOptions(context, prodcutConfigurationId, false, true);
				...
				for(int i=0;i<featureListMap.size();i++){
					...
					String strPrntType = (String) temp.get("from."+ConfigurationConstants.SELECT_TYPE);
					if(childOfCFs.contains(strPrntType)) {
						String strPrntName = (String) temp.get("from."+ConfigurationConstants.SELECT_NAME);
						String strChldName =(String) temp.get("to."+ConfigurationConstants.SELECT_NAME);
						features.append(strPrntName);
						features.append(":");
						features.append(strChldName);
						features.append(SelectConstants.cSelectDelimiter);
