    ...
    private void migrateRiskLevel(Context context, MapList objectList) throws Exception
    {
    	...
    	try{
    		...
    		while(objectListIterator.hasNext())
    		{
    			...
    			mqlLogRequiredInformationWriter("Migrating the value of 'Risk Level' for Risk " + objectId+" having impact:"+impact +" and probability:"+probability);
    			...
    			if("".equalsIgnoreCase(riskMatrixConfiguration) || riskMatrixConfiguration == null){
    				...
    				mqlLogRequiredInformationWriter("No RMC connected for this risk hence riskLevel BasedOn Default RMC is "+riskLevelBasedOnRMC);
    			} else {
    				...
    				mqlLogRequiredInformationWriter("RMC connected for this risk hence riskLevelBasedOnRMC is "+riskLevelBasedOnRMC);

    			}
    			
    			if(UIUtil.isNotNullAndNotEmpty(riskLevelBasedOnRMC) && !riskLevelBasedOnRMC.equalsIgnoreCase(riskLevel)){
    				
    				DomainObject riskObj = DomainObject.newInstance(context, objectId);
					
    				try {
    					Integer.parseInt(riskLevelBasedOnRMC);
    					riskObj.setAttributeValue(context, ATTRIBUTE_RISK_LEVEL, riskLevelBasedOnRMC);
    					mqlLogRequiredInformationWriter("Modified the value of Attribute 'Risk Level' for Risk " + objectId+" the new value is "+riskLevelBasedOnRMC);
    					
    					// Add object to list of converted OIDs
    					loadMigratedOids(objectId);
    				}catch(NumberFormatException e) {
    					mqlLogRequiredInformationWriter("Skipping object <<" + objectId + ">>, Risk Level in RMC is not Integer value");
    					
    					String comment = "Skipping object <<" + objectId + ">> Risk Level in RMC is not Integer value";
    					writeUnconvertedOID(comment, objectId);
    				}
