...
*/
   ...
   //R207 GBOM Part Updates end
	...
	public MapList getActiveGBOMStructure(Context context, String[] args)
        ...
        //limit filter and limit param removed 
		// Obj and Rel pattern
		String typePattern = "";
		String relPattern = ConfigurationConstants.RELATIONSHIP_GBOM;
		String strCustomPartMode = EnoviaResourceBundle.getProperty(context,
	...
	{
		StringList variantList = new StringList();
		Map tempMap = null;
		HashMap programMap = (HashMap) JPO.unpackArgs(args);
		MapList lstObjectIdsList = (MapList) programMap.get("objectList");
    ...
    public StringList excludeConnectedPartfamilies(Context context, String[] args) throws Exception
    {
    	...
    	for(int i=0;i<objectList.size();i++){
            ...
            excludeList.add(tempMap.get(ConfigurationConstants.SELECT_ID).toString());
        }
        excludeList.add(strObjectIds);
        
        // Called excludePFsWithReleasePhaseAsProduction() to get Part Family Ids which has Release Phase as "Production" (Default Part Type == policy_ECPart).
        // That need to be excluded from Product >> GBOM >> Add Existing Part Family
        // Comment Code below in R2018x HF1 as Production Phase is allowed in Product >> GBOM >> Add Existing Part Family
        /*StringList excludePFList = excludePFsWithReleasePhaseAsProduction(context, args);
        for(Object strPFId : excludePFList)
        {
        	excludeList.add((String)strPFId);
        }*/
        return excludeList;
    }
   ...
   */
    		...
    		if(!mapListTopLevelParts.isEmpty()){
    			...
    			for(int j=0; j<slTopLevelParts.size();j++){
    				...
    				if(!mapListParts.isEmpty()){
    					MapList slEBOMParts= new MapList(mapListParts.size());
   ...
   public List showLibraryPath(Context context, String[] args) throws Exception
   {
       ...
       StringList lstLibrartPath = new StringList();
       try {
     ...
     */
     ...
     /**
  	       ...
  	       //For Replace GBOM
  			...
  			for (int i = 0; i < objectList.size(); i++)
  			{
  				...
  				if(repalce)
  				{
  					...
  					String strCustomIconImage = new String();
  					String strGBOMReplaceTooltip = new String();
  					if(sourceType.equalsIgnoreCase(ConfigurationConstants.TYPE_PART) || prdSubTypes.contains(sourceType)){
  						strCustomIconImage = EnoviaResourceBundle.getProperty(context,"SummaryActions.ActionIcon.LogicalFeature_GBOM_Replace_Part");
  						strGBOMReplaceTooltip = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxConfiguration.ToolTip.LogicalFeature_GBOM_ReplacePart",context.getSession().getLanguage());
  					}else if(sourceType.equalsIgnoreCase(ConfigurationConstants.TYPE_PART_FAMILY)){
  						strCustomIconImage = EnoviaResourceBundle.getProperty(context,"SummaryActions.ActionIcon.LogicalFeature_GBOM_Replace_PartFamily");
  						strGBOMReplaceTooltip = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxConfiguration.ToolTip.LogicalFeature_GBOM_ReplacePartFamily",context.getSession().getLanguage());
  					}
  					strBufferIcons.append("<a href=\"javascript:showReplaceBySelector("+					
  							"'"+XSSUtil.encodeForXML(context,strobjectId)+"',"
  							+"'"+XSSUtil.encodeForXML(context,parentOID)+"',"
  							+"'"+XSSUtil.encodeForXML(context,strTypes)+"',"
  							+"'"+XSSUtil.encodeForXML(context,policyType)+"',"
  							+"'"+XSSUtil.encodeForXML(context,partState)+"',"
  							+"'"+XSSUtil.encodeForXML(context,strTimeStamp)+"',"
  							+"'"+XSSUtil.encodeForXML(context,strConnectionId)+"'"
  							+ ")\"><img border=\"0\" src=\"images/"+strCustomIconImage+"\"");
	  				strBufferIcons.append(" TITLE=\"");
	  				strBufferIcons.append(strGBOMReplaceTooltip);
	  				strBufferIcons.append("\"");
	  				strBufferIcons.append("/>");
	  				strBufferIcons.append("</a>");	
               }
  	 ...
  	 */
   	    ...
   	    */
   		...
   		public Map createPartFamilyJPO(Context context, String[] args) throws Exception {
   			...
   			try {
   				...
   				String strSymbolicTypeName           = FrameworkUtil.getAliasForAdmin(context, DomainConstants.SELECT_TYPE, sDefaultPartType, true);
   				if(strSymbolicTypeName != null && !"".equals(strSymbolicTypeName)){
   					sDefaultPartType = strSymbolicTypeName;
   				}
   		...
   		public Map getReleasePhase(Context context, String[] args) throws Exception {
   			...
   			String strObjType  = domObj.getInfo(context, DomainConstants.SELECT_TYPE);
   		...
   		public void updateDefaultPartType(Context context, String[] args)throws Exception{
   			try{
   				...
   				String strSymbolicTypeName           = FrameworkUtil.getAliasForAdmin(context, DomainConstants.SELECT_TYPE, sType, true);
   				if(strSymbolicTypeName != null && !"".equals(strSymbolicTypeName)){
   					sType = strSymbolicTypeName;
   				}
	    ...
	    /**This method is used to exclude the Part Family which has Release Phase as "Production"(Default Part Policy == policy_ECPart).
	    * This is excludeOID program.
	    * 
	    * @param context
	    *        The ematrix context object
	    * @param args
	    *         String[]
	    * @return StringList
	    *         List of Part Family Ids which has to be excluded.
	    * @throws
	    *        Framework Exception
	    * @since
	    *        R419.HF6                
	    */
	    @com.matrixone.apps.framework.ui.ExcludeOIDProgramCallable
	    public static StringList excludePFsWithReleasePhaseAsProduction(Context context, String[] args)throws FrameworkException
	    {
	       StringList excludePFList = new StringList();
	       try
	       {
	    	  Map programMap = (Map) JPO.unpackArgs(args);
	          String strObjectIds = (String)programMap.get("parentOID");
	          ...
	          if(strObjType != null && lstPRDChildTypes.contains(strObjType))
	          {
			      StringList lstObjSelect = new StringList();
			      lstObjSelect.add(DomainConstants.SELECT_ID);
			      lstObjSelect.add(ConfigurationConstants.ATTRIBUTE_DEFAULT_PART_POLICY);
			      String strWhereExpression = "attribute["+ ConfigurationConstants.ATTRIBUTE_DEFAULT_PART_POLICY + "] == policy_ECPart";
			      MapList mapListOfPFs = DomainObject.findObjects(context, ConfigurationConstants.TYPE_PART_FAMILY, "*", strWhereExpression, lstObjSelect);
			      if(mapListOfPFs != null && mapListOfPFs.size()>0)
			      {
			      	for(int j = 0; j < mapListOfPFs.size(); j++)
			      	{
			         	HashMap mapPF = (HashMap) mapListOfPFs.get(j);
			        	excludePFList.add((String)mapPF.get(DomainConstants.SELECT_ID));
			        }
			      }
	          }
			  return excludePFList;
	       }
	       catch(Exception e){
	    	  e.printStackTrace();
	          throw new FrameworkException(e.getMessage());
	    ...
	    /** This method is used to check if FTR(Variant Management Foundation)is installed or not.
	     *  @param context
	     *         the ematrix context object.
	     *  @param args
	     *         String[]
	     *  @return 
	     *          true - If FTR is installed. Phase field on type_PartFamily Web-Form will be available.
	     *          false - If FTR is not-installed. Phase field on type_PartFamily Web-From will not be available. 
	     *  @throws 
	     *          FrameworkException
	     *  @since
	     *          R2017x_HF7        
	     */
	    public static boolean displayPhaseField(Context context, String[] args)throws Exception
	    {
	    	boolean isFTRInstalled = false;
	    	try
	    	{
	    		isFTRInstalled = FrameworkUtil.isSuiteRegistered(context,"appVersionVariantConfiguration",false,null,null);
	    	}
	    	catch(Exception e)
	    	{
	    		//e.printStackTrace();
	    	...
	    	return isFTRInstalled;
	    }
	    
	    /* This method is used to get Release Phase value of Part Family on Part Family Web-Form in View and Edit mode.
	     * @param context
	     *        the ematrix context object
	     * @param args
	     *        String[]
	     * @return
	     *        It will return the Release Phase (Development/Production) value of Part Family.
	     * @throws
	     *         FrameworkException
	     * @since
	     *         R2017x_HF7
	     */
	    public static String getReleasePhaseForPF(Context context, String[] args)throws Exception
	    {
	    	String strLanguage = context.getSession().getLanguage();
	    	String strPFPhase  = DomainObject.EMPTY_STRING;
	    	try
	    	{
	    		HashMap programMap = JPO.unpackArgs(args);
		    	HashMap requestMap = (HashMap)programMap.get("requestMap");
		    	String  strPFId    = (String) requestMap.get("objectId");
		    	if(ProductLineCommon.isNotNull(strPFId))
		    	{
		    		DomainObject domObject = new DomainObject(strPFId);
		    		String strPhase = domObject.getInfo(context, "attribute["+ ConfigurationConstants.ATTRIBUTE_DEFAULT_PART_POLICY +"].value");
		    		if(ConfigurationConstants.RANGE_VALUE_DEFAULT_PART_POLICY_EC_PART.equalsIgnoreCase(strPhase)){
		    		  strPFPhase = EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Production",strLanguage);	
		    		}else if(ConfigurationConstants.RANGE_VALUE_DEFAULT_PART_POLICY_DEVELOPMENT_PART.equalsIgnoreCase(strPhase)){
		    		  strPFPhase = EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Development",strLanguage);	
		    		}
		    	}
	    	}
	    	catch(Exception e)
	    ...
	    /* This method is used to update/modify the Release Phase value of Part Family in Edit Mode.
	     * @param context
	     *        the ematrix context object
	     * @param args
	     *        String[]
	     ...
	     *           
	     */
	    public static void updateReleasePhaseForPF(Context context, String[] args)throws Exception
	    {
	    	...
	    	try
	    	{
	    		HashMap programMap    = JPO.unpackArgs(args);
		    	HashMap paramMap      = (HashMap)programMap.get("paramMap");
		    	String  strPFId       = (String) paramMap.get("objectId");
		    	String  strPFNewPhase = (String) paramMap.get("New Value");
		    	if(ProductLineCommon.isNotNull(strPFId)){
		    		DomainObject domObject = new DomainObject(strPFId);
		    		if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_PRODUCTION.equalsIgnoreCase(strPFNewPhase)){
		    		   domObject.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_DEFAULT_PART_POLICY, ConfigurationConstants.RANGE_VALUE_DEFAULT_PART_POLICY_EC_PART);
		    	    }else if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_DEVELOPMENT.equalsIgnoreCase(strPFNewPhase)){
		    	       domObject.setAttributeValue(context, ConfigurationConstants.ATTRIBUTE_DEFAULT_PART_POLICY, ConfigurationConstants.RANGE_VALUE_DEFAULT_PART_POLICY_DEVELOPMENT_PART);
		    	    }   	
		    	}
	    	}
	    	catch(Exception e)
