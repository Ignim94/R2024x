...
*/
/*
* @quickreview GVC LVA 03:15:2024 IR-1231204/IR-1231206 Exclude objects in Obsolete states from the implemented logic when a Chapter is promoted to Release
* @quickreview CGA5 LVA  22:12:12 IR-915922 Implement Link re routing moved to LWCServices from old VPLMJCoreModeler.
...
2023x	AGT11	  LVA		13:10:2023  FUN130134 - Enable Non Linear Revision for all Requirement corpus type
*************************************************************************************************************/
/**
...
//END : LX6 IR-251026V6R2014x NHIV6R-037278: Duplication of Tests Case and Use case is KO when None option is selected for anyone of them		   
       ...
       throws Exception
   {
        ...
        try
       {
           ...
           String strObsoleteChapter = "";
           String strObsoleteComment = "";
           String strObsoleteReqSpec = "";		   
           ...
           if(domReq.isKindOf(context,(RequirementsUtil.getChapterType(context))))
           {
        	   ...
        	   strObsoleteChapter = FrameworkUtil.lookupStateName(context, strChapterPolicy, strSymbObsoleteState);
        	   strObsoleteComment = FrameworkUtil.lookupStateName(context, strCommentPolicy, strSymbObsoleteState);
               strObsoleteReqSpec = FrameworkUtil.lookupStateName(context, strSpecificationPolicy, strSymbObsoleteState);			   
           }
           ...
           // GVC:3:13:2024 as per query parameters, the map contains only sub objects of type Chapter and Comment
           for(int i=0; i<mapSubObjects.size(); i++)
           {
			   ...
			   String strCurrentMaturity = domSubObj.getCurrentState(context).getName();

               // GVC:3:13:2024 IR-1231204-3DEXPERIENCER2023x and IR-1231206-3DEXPERIENCER2023x
			   // This code is changing state of  all child items to release state without checking any signature or any promote/demote logic
			   // Tactical fix to not change state of Obsolete items, the logic of automatic promotion to release should be completely reworked.
			   if (domSubObj.isKindOf(context,(RequirementsUtil.getChapterType(context))) && !strCurrentMaturity.equals(strObsoleteChapter))					   
                   domSubObj.setState(context, strReleaseChapter);
               else if (!strCurrentMaturity.equals(strObsoleteComment))			   
        ...
        throws Exception
    {
        ...
        try
        {
            ...
            DomainObject domLastRev = RequirementsCommon.getLastRevision(context, strObjectId);
       ...
       throws Exception
    {
           ...
           List<LWCEvent> ufEvents = new ArrayList<LWCEvent>();
           // unpack the incoming arguments into a HashMap called 'programMap'
           HashMap programMap = (HashMap) JPO.unpackArgs(args);
           HashMap paramMap = (HashMap) programMap.get("paramMap");
           ...
           try
           {
              ...
              if(addObjectIdList.size()>0)
              {
            	  ...
            	  //START : LX6 : recycling of a relationship
                  newRelIds = doEditOperation(context,addPasteAboveObjectId,addMarkupPasteAboveList,addPasteAboveRelId,strParentObjectId,PASTEABOVE,"add",recycleRelList,ufEvents,strRootId);
                  //END : LX6 : recycling of a relationship
                 ...
                 //invoke the method when markup is add "paste-below" to get the sorted maplist which contains map with the sequenceOrder, objectId and rowId key/value pairs
                  //START : LX6 : recycling of a relationship
                  newRelIds = doEditOperation(context,addPasteBelowObjectId,addMarkupPasteBelowList,addPasteBelowRelId,strParentObjectId,PASTEBELOW,"add",recycleRelList,ufEvents,strRootId);
                  //END : LX6 : recycling of a relationship
                  for(int i = 0; i < newRelIds.size(); i++){
                 ...
                 //invoke the method when markup is add "paste-as-child" to get the sorted maplist which contains map with the sequenceOrder, objectId and rowId key/value pairs
                  //START : LX6 : recycling of a relationship
                  newRelIds = doEditOperation(context,addPasteAsChildObjectId,addMarkupPasteAsChildList,addPasteAsChildRelId,strParentObjectId,PASTEASCHILD,"add",recycleRelList,ufEvents,strRootId);
                  //END : LX6 : recycling of a relationship
                  for(int i = 0; i < newRelIds.size(); i++){
              ...
              if(reseqObjectIdVector.size()>0)
              {
                 ...
                 //invoke the method when markup is resequence "paste-above" to get the sorted maplist which contains map with the sequenceOrder, objectId and rowId key/value pairs
                  doEditOperation(context,reseqPasteAboveObjectId,reseqMarkupPasteAboveList,reseqPasteAboveRelId,strParentObjectId,PASTEABOVE,"resequence",recycleRelList,ufEvents,strRootId);
                 ...
                 //invoke the method when markup is resequence "paste-below" to get the sorted maplist which contains map with the sequenceOrder, objectId and rowId key/value pairs
                  doEditOperation(context,reseqPasteBelowObjectId,reseqMarkupPasteBelowList,reseqPasteBelowRelId,strParentObjectId,PASTEBELOW,"resequence",recycleRelList,ufEvents,strRootId);
                 ...
                 doEditOperation(context,reseqPasteAsChildObjectId,reseqMarkupPasteAsChildList,reseqPasteAsChildRelId,strParentObjectId,PASTEASCHILD,"resequence",recycleRelList,ufEvents,strRootId);
   ...
   private List<String> getAttribute(List<String[]> strFullPath, String Attribute)
   {
     ...
     * @since RequirementsManagement V6R2008-2.0
     */
    private Vector doEditOperation(Context context, ArrayList objIds, StringList markupList, ArrayList relIds,String parObjId, String markup, String operation, StringList recycleRelIds, List<LWCEvent> ufEvents, String RootId)
       throws Exception
    {
       try
       {
          ...
          for (int counter = 0 ; counter < objectSize; counter++)
          {
           ...
           try {
             ...
             if(doReconcile.equalsIgnoreCase("true") && !isTestCase)
             {
                 ...
                 {
                	 try {
                       int size = Math.max(OldStacks.size(), NewStacks.size());
                       // Save Point , In case doReconcileUsingLWC , we have to revert back the changes
                       // on context done by logModificationUsingLWC , to commit the transaction.
                       ContextUtil.setSavePoint(context, "BeforeReconcile");
                       for (int ii = 0; ii < size; ii++) {
							//START LX6 FUN054695 ENOVIA GOV TRM Revision refactoring
                          ILReconciliationServices.logModificationUsingLWC(context,
	    								  OldStacks.get(ii),
	    								  NewStacks.get(ii),
                       ...
                       if (!ufEvents.isEmpty()) {
                          // Call to runMerge
                          ILReconciliationServices.doReconcileUsingLWC(context);
                       }

    				} catch (MatrixException e) {
    					e.printStackTrace();
                  // Warning - If doReconcileUsingLWC fails means implement link will not be re -
						// routed to the new path.In that case modification done by logEventsLWC has to
						// be reverted back from the context to the commit the next transactions.
						// this will abort the changes done on the context to the save point.
						ContextUtil.abortSavePoint(context, "BeforeReconcile");
    				}
                 } 
             }
