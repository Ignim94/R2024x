...
//            ex.printStackTrace();   
 ...
 //     System.out.println(sbURL.toString());
    ...
    /**
     ...
     protected MapList getAllProjectIssues(Context context, String[] args, String Filter) throws Exception
     {
        ...
        if("true".equalsIgnoreCase(isCallForProgramRIO)) {
        	DebugUtil.debug("Loading Indexed issues for Program-->RIO-->Issues");
        if("Active".equalsIgnoreCase(Filter)){
        	returnIssueList = getAllIssues(context, taskIds, true, false);
        }else if("Closed".equalsIgnoreCase(Filter)){
        	returnIssueList = getAllIssues(context, taskIds, false, true);
        }else if("All".equalsIgnoreCase(Filter)){
        	returnIssueList = getAllIssues(context, taskIds, true, true);
        }
     ...
     * @throws Exception
     */
    private static MapList getAllIssues(Context context, Set<String> affectedItemIdSet , boolean active , boolean closed) throws Exception {
    	...
    	MapList issues = new MapList();
	   	try {
	   		 ...
	   		 for(String affectedItemId : affectedItemIdSet) {
	...
	private static MapList getIssuesByQuery(Context context,IXIssueQueryParameters queryParameters)  throws Exception{
		 ...
		 for(IXBrowseIssue browserIssue : BrowserIssues) {
			 ...
			 issue.put(DomainConstants.SELECT_ID, browserIssue.getM1Id());
			 issues.add(issue);
		 }
		 return issues;
	}
