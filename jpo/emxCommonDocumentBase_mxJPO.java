	...
	*/
	...
	public void actionChangePOV(Context context, String args[]) throws Exception {
		...
		if(!policy.equals("Version")){		
			...
			String activeVersion = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump $3", args[0], "relationship[Active Version].to.physicalid", "|"); 

			if(!(activeVersion.isEmpty() || activeVersion == null)){
				String[] listActiveVersion=activeVersion.split("\\|");	

				try {
					ContextUtil.pushContext(context, PropertyUtil.getSchemaProperty(context, "person_UserAgent"), DomainConstants.EMPTY_STRING, DomainConstants.EMPTY_STRING);
					contextPushed = true;

					for(int i=0;i<listActiveVersion.length;i++){

						if(!newCollSpaceName.isEmpty()){
							MqlUtil.mqlCommand(context,"modify bus $1 project $2",listActiveVersion[i],newCollSpaceName);
						}

						if(!newOrgName.isEmpty()){
							MqlUtil.mqlCommand(context,"modify bus $1 organization $2",listActiveVersion[i],newOrgName);
						}

						if(!newOwnerName.isEmpty()){
							MqlUtil.mqlCommand(context,"modify bus $1 owner $2",listActiveVersion[i],newOwnerName);
						}
					}
				} catch (Exception e) {
					throw e;
				...
				for (String strLog : slObjectLog) {
					if (strLog.contains("connect Active Version to")) {
						...
						try {
							...
							_log.trace("strLog strVersionDocTNR is  : {}", strVersionDocTNR);
							StringList slVersionTNR = FrameworkUtil.split(strVersionDocTNR, " ");
							String strResult = MqlUtil.mqlCommand(context, "temp query bus $1 $2 $3 select $4 $5 dump $6", slVersionTNR.get(0), slVersionTNR.get(1), slVersionTNR.get(2), "physicalid","attribute[Title]", "|");
							StringList slResult = FrameworkUtil.split(strResult, "|");
