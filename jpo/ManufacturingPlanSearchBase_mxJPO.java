	...
	public StringList getMPofAllRevisions(Context context,String[] args)throws Exception
	{
		...
		try
		{
			...
			for(int i=0;i<revisionsList.size();i++){
				...
				/*int iLevel = ${CLASS:ManufacturingPlanBase}.getLevelfromSB(context,args);*/
				int iLevel = JPO.invoke(context, "ManufacturingPlanBase", null, "getLevelfromSB", args);
...
//			}
	...
	public StringList getManufacturingPlansForDerivationCreate (Context context, String[] args) throws Exception {
		...
		try {
			...
			lstMultiValueList.add(strMPIDSelectable);
			MapList mlMPList = DomainObject.getInfo(context, (String[])slIncludeIds.toArray(productsidsArray), objectSelects, lstMultiValueList);
	...
	public StringList getManufacturingPlansForRevisionCreate (Context context, String[] args) throws Exception {
		...
		try {
			...
			StringList lstMultiValueList = new StringList();
			lstMultiValueList.add(strMPIDSelectable);
			lstMultiValueList.add(strMainDerivedSelectable);
			MapList mlMPList = DomainObject.getInfo(context, (String[])slIncludeIds.toArray(productsidsArray), objectSelects, lstMultiValueList);
