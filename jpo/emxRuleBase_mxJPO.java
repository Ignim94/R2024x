...
public class emxRuleBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    /**
     * Trigger called on ATTRIBUTE_MANDATORYRULE update; 
     * Rule Create with /without Mandatory Rule Attribute set, also called on Edit Rule Mandatory Rule Attribute; 
     * 
     * 
     * @param context
     * @param args
    ...
    public void inheritMandatoryRulesOnModifyAttribute(Context context,String[] args) throws Exception {
    	...
    	String strRelId = args[0];//Rel Id
    	DomainRelationship domRel = new DomainRelationship(strRelId);
    	String strAttrVal = domRel.getAttributeValue(context,ConfigurationConstants.ATTRIBUTE_MANDATORYRULE);//Mandatory Rule
        //-----------------------------------------------------------------------------------------------------------------------------
    	// RuleContext- is empty in Create/Edit Rule Case- only in add existing this is set to "Create"- and in that case trigger is skipped
        //-----------------------------------------------------------------------------------------------------------------------------    	
        ...
        //-----------------------------------------------------------------------------------------------------------------------------
        // Mandatory Rule changed from No to Yes
        //-----------------------------------------------------------------------------------------------------------------------------        
        if (!isCreate && (strAttrVal != null && !"".equals(strAttrVal)&& strAttrVal.equalsIgnoreCase("Yes"))){
        	...
        	StringList slFromObjIds = (StringList) htRelData.get(DomainRelationship.SELECT_FROM_ID);//Rule's Context Object ID
        	StringList slToObjIds = (StringList) htRelData.get(DomainRelationship.SELECT_TO_ID);//Rule ID
        ...
        //-----------------------------------------------------------------------------------------------------------------------------
        // Mandatory Rule changed from Yes to No
        //-----------------------------------------------------------------------------------------------------------------------------         
        if (strAttrVal != null && !"".equals(strAttrVal) && strAttrVal.equalsIgnoreCase("No")) {
            ...
            //TODO- need to Revisit this getParents API Call
            ...
            StringList lstObjectSelects = new StringList(DomainConstants.SELECT_ID);
            //get all 1st level rel from context object exclude SubPL for PL case get Model in that case
            ...
            //-------------------------------------------------------
            //If From Object is of PL- then specific loop for SUB PL        
            //-------------------------------------------------------            
            if (mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_PRODUCT_LINE) ) {
                ...
                for (int iCnt = 0; iCnt < childObjListPl.size(); iCnt++) {
                    ...
                    StringList lstRelSelects = new StringList(DomainRelationship.SELECT_ID);
                    String strWhere = "id==\"" + strRuleId + "\"";
                    //Check if SUB PL has already has same rule which Mandatory Rule getting set to No
                    MapList relList = domObj.getRelatedObjects(context,strRelType, strRuleType, strObjectSelectablesPV,(StringList) lstRelSelects, false, true, (short) 1,
                            strWhere, DomainConstants.EMPTY_STRING, 0);
                    //if SUB PL also has same Rule
                    for (int count = 0; count < relList.size(); count++) {
                        Hashtable htTypeObjMap = (Hashtable) relList.get(count);
                        String strrelId = (String) htTypeObjMap.get(DomainRelationship.SELECT_ID);// get SUBPL->RUle rel ID
                        //set Inherited attribute to False;
                        domRel = new DomainRelationship(strrelId);
                        domRel.openRelationship(context);
                        domRel.setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_INHERITED,"False");
                        domRel.closeRelationship(context, true);
                        //Inherited From Rel to be disconnected
                        String strMqlCmd1 = "print connection $1 select $2 dump $3";
                        StringBuffer selectable = new StringBuffer();
                        selectable.append("tomid[").append(ConfigurationConstants.RELATIONSHIP_INHERITED_FROM).append("].id");
                        ...
                        MqlUtil.mqlCommand(context, strMqlCmd2, true,tomidId);//TODO- will need check if tomidID null??
                    }//For all sub PL's Rule
                }//For all Sub PL
            }// For SUB PL iteration for Rule check -
            
            //--------------------------------------------------------
            //For other Child Object for corresponding context Object       
            //--------------------------------------------------------            
            try{
            	DomainObject domObj = new DomainObject();
            	for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
            		...
            		StringList lstRelSelects = new StringList(DomainRelationship.SELECT_ID);
            		String strWhere = "id==\""+ strRuleId + "\"";
            		//Check if child Object has already has same rule which Mandatory Rule getting set to No
            		...
            		for (int count = 0; count < relList.size(); count++) {
            			Hashtable htTypeObjMap = (Hashtable) relList.get(count);
            			String strrelId = (String) htTypeObjMap.get(DomainRelationship.SELECT_ID);// get Childobject->RUle rel ID
            			//set Inherited attribute to False;
            			domRel = new DomainRelationship(strrelId);
            			domRel.openRelationship(context);
            			domRel.setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_INHERITED,"False");
            			...
            			selectable3.append("tomid[").append(ConfigurationConstants.RELATIONSHIP_INHERITED_FROM).append("].id");                    
            			...
            			String strMqlCmd2 = "disconnect connection $1";
            			MqlUtil.mqlCommand(context, strMqlCmd2, true,tomidId);//TODO- will need check if tomidID null??
            		}//For all Child Object's Rule
            	}//For all Child Object
            }// For Child Object iteration for Rule check -
            catch(Exception e){
     ...
     * 
     * Updated for 2018x FD02- used isCurrentInAllowedState to check context object state to invalid states in Properties file 
     *
     * @param context
     *            the eMatrix <code>Context</code> object.
     * @param args
    ...
    public void ruleInheritanceOnEdit(Context context,  String args[]) throws Exception {
    	try{
    		...
    		DomainObject domContextObject = new DomainObject(strContextId);
    		...
    		//get parenttype for from context object- TODO - Need to revisit getParents API?
    		BusinessType bType = new BusinessType(strFromObjType, new Vault(""));
    		StringList slListParents = bType.getParents(context);
    		String strParentType = "";
    		if (slListParents.size() != 0) {
    		...
    		StringList subTypes = new StringList() ;
    		String strChildType = "";
    		String strChildRel = "";

    		String strChildRelsForEdit="";
    		StringBuffer sbChildRelForEdit = new StringBuffer();
    		//depending on context object type set rel and object to iterate-
    		if (mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_PRODUCT_LINE) ) {
    			// for get Related to Process all the Objects at once
    			// For PL- it can be SUB PL, Model, PRroduct and Product Variant 
    			...
    			// For Model- it can be PRroduct and Product Variant 
    			strChildType = ConfigurationConstants.TYPE_PRODUCTS;
    			...
    			// For Product- it can be Product Variant					
    			strChildType = ConfigurationConstants.TYPE_PRODUCTS;
    			strChildRel = ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION;
    		...
    		//To Skip PV if Context Product is in Invalid State list--
    		StringBuilder typeKindPV = new StringBuilder(50);
    		typeKindPV.append("type.kindof[");
    		typeKindPV.append(ConfigurationConstants.TYPE_PRODUCT_VARIANT);
    		typeKindPV.append("]");
    		lstObjectSelects.addElement(typeKindPV.toString());
    		lstObjectSelects.add("to["+ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION+"].from.current");
    		lstObjectSelects.add("to["+ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION+"].from.policy");

    		StringList relSelects = new StringList(2);
    		relSelects.add(DomainConstants.SELECT_RELATIONSHIP_ID);
    		relSelects.add(DomainConstants.SELECT_RELATIONSHIP_NAME);
    		//get corresponding child object to which Rule will be inherited
    		MapList childObjList = domContextObject.getRelatedObjects(context,strChildRel, strChildType, (StringList) lstObjectSelects, relSelects,false, true, (short) 0, DomainConstants.EMPTY_STRING,
    				DomainConstants.EMPTY_STRING,0);
    		...
    		String strInvalidStates = EnoviaResourceBundle.getProperty(context, "emxConfiguration.Attribute.MandatoryRules.InheritanceNotAllowedStates");
    		...
    		for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
    			...
    			bInvalidState=ConfigurationUtil.isCurrentInStateList(context,sPolicy,sObjState,strInvalidStates);
    			if (bInvalidState == true) {
    				continue;// continue with next object for inheritance- if it is in Invalid state for inheritance.
    			}
    			//-------------------------------------------------------------------------------
    			//Check if it PV object Check if context Product is in Invalid states------------
    			//-------------------------------------------------------------------------------
    			String strTypeKindPV = (String) htChildObjMap.get(typeKindPV.toString());
    			if(strTypeKindPV.equalsIgnoreCase("true")){
    				String sContextPRDObjState = (String) htChildObjMap.get("to["+ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION+"].from.current");
    				String sContextPRDPolicy = (String) htChildObjMap.get("to["+ConfigurationConstants.RELATIONSHIP_PRODUCT_VERSION+"].from.policy");
    				bInvalidState=ConfigurationUtil.isCurrentInStateList(context,sContextPRDPolicy,sContextPRDObjState,strInvalidStates);
    				if (bInvalidState == true) {
    					continue;// continue with next object for inheritance- if PV's Context Product is in Invalid state 
    				}
    			}
    			//--------------------------------------------------------------------------------
    			domObj.setId(strObjId);
    			// Check if the Rule is already connected with child object						
    			StringList lstRelSelects = new StringList(DomainRelationship.SELECT_ID);
    			String strWhere = "id==\""+ strRuleId + "\"";                
    			// Get only the details of the Rule Object connected to the Child Object 
    			...
    			if(relList.size()==0){
    				...
    				//Update new Rel with RELATIONSHIP_INHERITED_FROM Connection  
    				...
    				String strMqlCmd2 = "add connection $1 fromrel $2 torel $3";
    				MqlUtil.mqlCommand(context, strMqlCmd2, true,ConfigurationConstants.RELATIONSHIP_INHERITED_FROM,strRuleRelId,relId);
    				strRuleRelId = relId;
    				...
    				for (int j = 0; j < relList.size(); j++) {
    					...
    					//TODO- COULD NOT UNDERSTAND THIS CASE TO HAVE BELOW CALL HERE-
    					...
    					if (subTypes.contains(strchildType) && strChildRelsForEdit.contains(strchildRel) && MlRelIds.isEmpty()){
    						...
    						domRelationship.setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, "Yes");
    						MqlUtil.mqlCommand(context, "trigger $1", true, "on");
    						//Update Rel with RELATIONSHIP_INHERITED_FROM Connection                        		
    						String strMqlCmd2 = "add connection $1 fromrel $2 torel $3";
    						MqlUtil.mqlCommand(context, strMqlCmd2, true,ConfigurationConstants.RELATIONSHIP_INHERITED_FROM,strRuleRelId,relId);
    					}
    				}
    ...
    public void ruleInheritanceOnCreate(Context context,  String args[]) throws Exception {
    	try{
    		...
    		for (int iCnt = 0; iCnt < childObjList.size(); iCnt++) {
    			...
    			//-----------------------------------------------------------------------------------------
    			// for child object which is not in invalid state- will process to connect rule with it
    			// update the attributes for inherited and Mandatory attribute
    			domObj.setId(strObjId);
    			domRelationship = DomainRelationship.connect(context,domObj, strRuleRelType, domBCR);
    			domRelationship.setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_INHERITED, "True");
    			domRelationship.setAttributeValue(context,ConfigurationConstants.ATTRIBUTE_MANDATORYRULE, "Yes");
    			//Connect Inherited From Rel
    			relData = (Hashtable)domRelationship.getRelationshipData(context,strListRel);
    			s2RelIds = (StringList) relData.get(DomainConstants.SELECT_ID);
    			relId = (String) s2RelIds.get(0);
    			String strMqlCmd2 = "add connection $1 fromrel $2 torel $3";
    			MqlUtil.mqlCommand(context, strMqlCmd2, true,ConfigurationConstants.RELATIONSHIP_INHERITED_FROM,strRelIdBCRContext,relId);
