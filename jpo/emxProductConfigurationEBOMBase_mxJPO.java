...
/*        String strFeatureId = domGbom.getInfo(context, "to["
       ...
       String strAtrDesignVariants = (String)objSelectsMap.get("to["
        ...
        for (int i = 0; i < invalidDVList.size(); i++)
        {
            ...
            {
                StringList PVList = new StringList();
  ...
  /**
	...
	protected ArrayList gbomParseRulesIdBased(Context context,
        ...
        /*MapList selectedOptionsMaplist = ${CLASS:emxBatchRuleProcessing}.getSelectedOptions(context,
                getId(context));*/
        Map mArgs = new HashMap();
    	mArgs.put("paramString",getId(context));
    	String[] newArgs = JPO.packArgs(mArgs);
    	MapList selectedOptionsMaplist = (MapList) JPO.invoke(context, "emxBatchRuleProcessing", null, "getSelectedOptions", newArgs, MapList.class);
                ...
                if (ruleEvaluationRequired)
                {
                	...
                	if(strUseDesignVariant != null && !strUseDesignVariant.equals("true"))
                    {
                    	if(strAttrRExp != null)
                    	{
                    		...
                    		pMap.put("strGBOMID", strGBOMID);
                    		pMap.put("strExpression", strExpression);
                    		pMap.put("selectedOptions", selectedOptions);
                    		mapArgs.put("paramMap", pMap);
                    		String[] newJPOArgs = JPO.packArgs(mapArgs);
                    		strExpResult = (String) JPO.invoke(context, "emxBatchRuleProcessing", null, "calculateExpressionIdBased", newJPOArgs, String.class);
                    	...
                    	if(strAttrRExp != null)
                    	{
                            ...
                            .calculateExpressionIdBased(context, strGBOMID,
                                    strExpression, selectedOptions);*/
                    		Map mMapArgs = new HashMap();
                    		Map pMap = new HashMap();
                    		pMap.put("strGBOMID", strGBOMID);
                    		pMap.put("strExpression", strExpression);
                    		pMap.put("selectedOptions", selectedOptions);
                    		mMapArgs.put("paramMap", pMap);
                    		String[] newJPOArgs = JPO.packArgs(mMapArgs);
                    		strExpResult = (String) JPO.invoke(context, "emxBatchRuleProcessing", null, "calculateExpressionIdBased", newJPOArgs, String.class);
                    	}
 ...
 */
    ...
    public Vector getInclusionRuleLinkForDisplay(Context context, String args[])throws Exception {
    	...
    	for (int i = 0; i < objectList.size(); i++) {
    		...
    		if(objectMap.get(ConfigurationConstants.SELECT_LEVEL).toString().equals("0")){
	    		...
	    		.append("mode=FromNonDecoupledEffectivity&amp;PCID="+XSSUtil.encodeForHTMLAttribute(context,strPCID))
	    		.append("&amp;LFId=")
    ...
    /**
     * This method displays link -which is used on click shows Variant Effectivity set.
     * @param context
     * @param args
     * @return
     * @throws Exception
     */
    public Vector getVariantEffectivityLinkForDisplay(Context context, String args[])throws Exception {
    	HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
    	MapList objectList = (MapList) parametersMap.get("objectList");
    	String strObjectId = "";
    	Vector vInclusionRuleLink = new Vector();
    	...
    	for (int i = 0; i < objectList.size(); i++) {
    		...
    		if(objectMap.get(ConfigurationConstants.SELECT_LEVEL).toString().equals("0")){
    			...
    			sIncLink = sIncLink.append("<a href=\"javascript:showDialog('")
    					.append("../configuration/GBOMRuleExpressionDialog.jsp?")
    					.append("mode=FromDecoupledVEffectivity&amp;PCID="+XSSUtil.encodeForHTMLAttribute(context,strPCID))
    					.append("&amp;LFId=")
    					.append(XSSUtil.encodeForHTMLAttribute(context,strObjectId))
    					.append("&amp;PartRelId=")
    					.append(XSSUtil.encodeForHTMLAttribute(context,strGBOMRelID));
     ...
     * 			If the operation fails
     */
    public boolean isDecouplingEnabled (Context context, String args[]) throws Exception {
    	return ConfigurationUtil.isDecouplingEnabled(context); 
    }
    /**
     * Are decoupling enabled?
     *
     ...
     * 			If the operation fails
     */
    public boolean isDecouplingDisabled (Context context, String args[]) throws Exception {
    	return !ConfigurationUtil.isDecouplingEnabled(context); 
    }
    /**
   ...
   /**
     ...
     */
     public Vector getActionIconsForDisplay(Context context, String args[])throws Exception {
         ...
         String i18ReplabeByGenerate = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByGenerateAsDefault",context.getSession().getLanguage());
         String i18ReplabeByAddExisting = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByAddExistingAsDefault",context.getSession().getLanguage());
         String i18ReplabeByCreateNew = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByCreateNewAsDefault",context.getSession().getLanguage());
         String i18Generate = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.GenerateAsDefault",context.getSession().getLanguage());
         String i18AddExisting = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.AddExistingAsDefault",context.getSession().getLanguage());
         String i18CreateNew = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.CreateNewAsDefault",context.getSession().getLanguage());
 ...
 String strLevel = (String)objectMap.get("level");
             ...
             {
            	 String strDuplicateParts=Duplicateparts.toString();
				 strDuplicateParts = strDuplicateParts.replace("[","") ;
				 strDuplicateParts = strDuplicateParts.replace("]","") ;
                 if (isECInstalled)
                 {
                     stbNameRev = stbNameRev
                             .append("<a><img src=\"../common/images/iconSmallCreatePart.png")
                 ...
                 stbNameRev = stbNameRev
                         .append("  <a><img src=\"../common/images/IconSmallAddExistingPart.png")
                         .append("\" border=\"0\"  align=\"middle\" ")
                         .append("TITLE=\"")
                 ...
                 if (partFamily.size() == 1)
                 {
                     ...
                     if (!("obsolete").equalsIgnoreCase(strPartFamilyState))
                     {
                         stbNameRev = stbNameRev.append("  <a href=\"javaScript:emxTableColumnLinkClick('");
									  ...
									  stbNameRev.append(strDuplicateParts);
									  ...
									  stbNameRev.append("<img border=\"0\" align=\"middle\" src=\"../common/images/IconSmallGenerateParts.png\" title=\""+i18Generate+"\"/></a>");
                     }
                 }
             }
             if (parts.size()>0)
             {
                 ...
                 if (isECInstalled)
                 {
                     stbNameRev = stbNameRev.append("<a><img src=\"../common/images/IconSmallReplacePartNewPart.png")
				                            .append("\" border=\"0\"  align=\"middle\" ")
				                            .append("TITLE=\"")
                 ...
                 stbNameRev = stbNameRev.append("  <a><img src=\"../common/images/IconSmallReplacePartExistingPart.png")
				                        .append("\" border=\"0\"  align=\"middle\" ")
				                        .append("TITLE=\"")
                 ...
                 if (partFamily.size() == 1)
                 {
                     ...
                     if (!("obsolete").equalsIgnoreCase(strPartFamilyState))
                     {
                         stbNameRev = stbNameRev.append("  <a href=\"javaScript:emxTableColumnLinkClick('");
									  ...
									  stbNameRev.append(XSSUtil.encodeForHTMLAttribute(context, strPartFamily));
									  stbNameRev.append("', '700', '600', 'true', 'listHidden', '')\">");
									  stbNameRev.append("<img border=\"0\" align=\"middle\" src=\"../common/images/IconSmallReplaceNewGeneratedPart.png\" title=\""+i18ReplabeByGenerate+"\"/></a>");
                     }
                 }
             }
     ...
     */
     public Vector getVisualCueForDisplay(Context context, String args[])throws Exception {
    	 ...
    	 try {
		   ...
		   if (ProductLineCommon.isNotNull(strCustomPartMode)
			   ...
			   strToolTip = "emxProduct.ToolTip.VisualCue.DuplicateParts";
		   } else {   						
			   strToolTip = "emxProduct.ToolTip.VisualCue.DuplicateParts";
		   }
    		 ...
    		 if(isOnlyProductConfiguration)
    		 {
    			 ...
    			 if(!("".equals(strTLPId) || "null".equals(strTLPId) || null==strTLPId))
    			 {
    				 ...
    				 if(isBOMDifferent)
    				 {
    					 ...
    					 .append(
    					 "  <img src=\"../common/images/CompareDifferences16.png")
    					 .append("\" border=\"0\"  align=\"middle\" ")
    					 .append("TITLE=\"")
	 ...
	 String strLevel = (String)objectMap.get("level");
    				 ...
    				 if (parts.size() >= 1)
    				 {
    					 ...
    					 .append(
    					 "  <a><img src=\"../common/images/DuplicateParts16.png")
    					 .append("\" border=\"0\"  align=\"middle\" ")
    					 .append("TITLE=\"")
    				 ...
    				 if(strResolvedPart!=null){
    					 ...
    					 if (!isLastRev)
    					 {
    						 stbVisualCue = stbVisualCue
    						 .append(
    						 "  <img src=\"../common/images/HigherRevision16.png")
    						 .append("\" border=\"0\"  align=\"middle\" ")
    						 .append("TITLE=\"")
    					 ...
    					 if ((current).equalsIgnoreCase(strObsolete))
    					 {
    						 stbVisualCue = stbVisualCue
    						 .append(
    						 "  <img src=\"../common/images/IconSmallObsoletePart.png")
    						 .append("\" border=\"0\"  align=\"middle\" ")
    						 .append("TITLE=\"")
    				 ...
    				 if (pc.checkForUsedInBOM(context, strObjectId,strResolvedPart,strLFParentId, parentId))
    				 {
    					 stbVisualCue = stbVisualCue
    					 .append(
    					 "  <img src=\"../common/images/UsedInBom16.png")
    					 .append("\" border=\"0\"  align=\"middle\" ")
    					 .append("TITLE=\"")
    					 .append(i18UsedInBOM)
    									 .append("\"").append("/>");
   ...
   /**
     ...
     * @author KXB
     */
     public StringList getPartRevisionForDisplay(Context context, String[] args)throws Exception{
    		HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
    		MapList objectList = (MapList) parametersMap.get("objectList");
    		ArrayList parts = new ArrayList();
    		StringList vRevision = new StringList();
...
String strLevel = (String)objectMap.get("level");
   ...
   /**
     ...
     * @since  R212
     */
    public StringList getPartStateForDisplay(Context context, String[] args) throws Exception {
            HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList) parametersMap.get("objectList");
            Map paramList = (Map) parametersMap.get("paramList");
    		String localeObj = paramList.get("localeObj").toString();
            ArrayList parts = new ArrayList();
            StringList vState = new StringList();
            StringList objListIDs = new StringList(objectList.size());
            Set<Integer> lst_gaps = new HashSet<Integer>( objectList.size() );
            //String gaps = "";      
            ...
            if (objListIDs.size() > 0) 
            {
            	StringList objSelects = new StringList(ConfigurationConstants.SELECT_CURRENT);
            			objSelects.add(ConfigurationConstants.SELECT_POLICY);
                String[] oidsArray = new String[objListIDs.size()];
                mapList = DomainObject.getInfo(context, (String[]) objListIDs
                        .toArray(oidsArray), objSelects);
            }
            Iterator itr = mapList.iterator();
            Map tempMap;String interNationalizedStateName;
            for (int j = 0; j < objectList.size(); j++) 
            {
            	boolean b_isAGap = lst_gaps.contains(j);
                /*int index = gaps.indexOf(""+j+"");
                ...
                {
                    tempMap = (Map) itr.next();
                    interNationalizedStateName = EnoviaResourceBundle.getStateI18NString(context, 
                    		(String) tempMap.get(ConfigurationConstants.SELECT_POLICY),
                    		(String) tempMap.get(ConfigurationConstants.SELECT_CURRENT),localeObj);
                    vState.addElement(interNationalizedStateName);
                } else 
                {
                	vState.addElement("");
                }
   ...
   /**
     ...
     * @since  R212
     */
     public StringList getPartTypeForDisplay(Context context, String[] args) throws Exception {
             HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
             MapList objectList = (MapList) parametersMap.get("objectList");
             ArrayList parts = new ArrayList();
             StringList vType = new StringList();
             StringList objListIDs = new StringList(objectList.size());
             Set<Integer> lst_gaps = new HashSet<Integer>( objectList.size() );
         	//String gaps = "";
         	//IR-148613V6R2013
...
String strLevel = (String)objectMap.get("level");
   ...
   /**
     ...
     * @since  R212
     */
     public StringList getUOMForDisplay(Context context, String[] args) throws Exception {
            HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
            MapList objectList = (MapList) parametersMap.get("objectList");
            ArrayList parts = new ArrayList();
            StringList vUOM = new StringList();
            StringList objListIDs = new StringList(objectList.size());
            Set<Integer> lst_gaps = new HashSet<Integer>( objectList.size() );
        	//String gaps = "";
        	//IR-148613V6R2013
   ...
   //Method added for Bug No. 377168
    public ArrayList getPartRuleData(Context context,ArrayList arrFeatureIds, String strProdVariantId) throws Exception
     {
        ...
        StringList listActiveDV = new StringList();
    ...
    private boolean isFeatureConnectedToProduct(Context context,
		...
		Map mpRelIds = rightExpObject.getInfo(context, objSel, lstMultiValueList);
		...
		for (int j = 0; j < mpRelIds.size(); j++) {
			StringList slLFRelIDs = (StringList) (mpRelIds.get("to["
					+ ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES
					+ "].physicalid"));
			allRelIDs.add(slLFRelIDs.toString());
			StringList slCFRelIDs = (StringList) (mpRelIds
					.get("to["
							+ ConfigurationConstants.RELATIONSHIP_CONFIGURATION_STRUCTURES
							+ "].physicalid"));
			allRelIDs.add(slCFRelIDs.toString());
		}
   ...
   private StringList getObjectRelPhyIdsFromMap(Context context,MapList mapListFeature) throws FrameworkException
   {
	   ...
	   if(mapListFeature!=null && !mapListFeature.isEmpty())
	   {
		   ...
		   for(int i=0;i<mapListFeature.size();i++)
		   {
			   ...
			   slTemp.add(tempMap.get("physicalid").toString());
			   ...
			   if(mlrelIDs.size()>0){
				   slTemp.add(((Map)(mlrelIDs.get(0))).get("physicalid").toString());
			   }
		   }
	   }
	   // Return StringList
   ...
   public Vector getCustomActionIconsForDisplay(Context context, String args[])throws Exception {
	   ...
	   boolean isUsedInBOM = false;
        boolean isECInstalled = false;
        isECInstalled = FrameworkUtil.isSuiteRegistered(context,"appVersionX-BOMEngineering", false, null, null);
        String i18Generate = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.GenerateAsCustom",strLanguage);
        String i18AddExisting = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.AddExistingAsCustom",strLanguage);
        String i18CreateNew = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.CreateNewAsCustom",strLanguage);
        String i18ReplabeByGenerate = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByGenerateAsCustom",context.getSession().getLanguage());
        String i18ReplabeByAddExisting = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByAddExistingAsCustom",context.getSession().getLanguage());
        String i18ReplabeByCreateNew = EnoviaResourceBundle.getProperty(context, SUITE_KEY,"emxProduct.ToolTip.ReplabeByCreateNewAsCustom",context.getSession().getLanguage());    
     ...
     */
     public Vector getPartUsageForDisplay (Context context, String args[])throws Exception {
         ...
         String strPartUsage = "";
         String strLanguage = context.getSession().getLanguage();
         String strDefault = EnoviaResourceBundle.getProperty(context, SUITE_KEY,
                 "emxConfiguration.Range.Part_Usage.Default",strLanguage);
...
public Vector displayQuantity(Context context, String args[])
 ...
 public Object getDuplicateParts(Context context, String[] args)throws Exception {
	...
	// Called ECM API to get Change Control value for Parts.
	HashMap partChangeControlledMap = new HashMap();
	HashMap programMap = new HashMap();
	programMap.put("objectList", vDuplicateParts);
	...
	// Prepare the Map which contains key as Part Id and value as Change Controlled value of Part.
	for(int i = 0; i < lstChangeControlled.size() ;i++){
		Map map = (Map)vDuplicateParts.get(i);
	    String partId = (String) map.get(DomainObject.SELECT_ID);
	    String changeControled = (String)lstChangeControlled.get(i);
	    partChangeControlledMap.put(partId, changeControled);
	}
	
	// Replace the Change Controlled value of Evaluated Parts
	for(int j=0; j < vDuplicateParts.size(); j++)
	{
		Map partMap = (Map) vDuplicateParts.get(j);
		String strPartId = (String) partMap.get(DomainConstants.SELECT_ID);
		if(partChangeControlledMap.containsKey(strPartId)){
			partMap.put(ConfigurationConstants.SELECT_ATTRIBUTE_CHANGE_CONTROLLED, (String)partChangeControlledMap.get(strPartId));
		}
	}
...
throws FrameworkException {
	...
	try {
		...
		objSelects.addElement(ConfigurationConstants.SELECT_ATTRIBUTE_RELEASE_PHASE);
		objSelects.addElement(ConfigurationConstants.SELECT_ATTRIBUTE_CHANGE_CONTROLLED);
		...
		//------------------------------------------------------------
		//START of GBOM Part Evaluation Modification
		//------------------------------------------------------------
		
		StringList relSelectable = new StringList();
        StringList lstMultiValueList = new StringList();
		relSelectable.add("from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id");
		lstMultiValueList.add("from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id");
		MapList lfDVIdList = DomainObject.getInfo(context, new String[]{strFeatureId}, relSelectable, lstMultiValueList);
		
		//Mapping LF IDs to fetched DV IDs list
		HashMap lfDVMap = new HashMap();
		lfDVMap.put(strFeatureId, lfDVIdList.get(0));
		
		//Get all the GBOM Rels in an array which will be later used for single getInfo call to get variant effectivity usage details
		StringList gbomRelIdList = new StringList();  
		for(int i = 0; i < evaluatedParts.size(); i++){
			Map gbomRelInfo = (Map) evaluatedParts.get(i);
			String gbomRelId = (String) gbomRelInfo.get("id[connection]");
			gbomRelIdList.add(gbomRelId);
		}
		
		//get Design Variant ids using GBOM Rels
		String[] gbomRelIDsArray = new String[gbomRelIdList.size()];
		StringList relSel = new StringList();
		relSel.add("frommid[" + ConfigurationConstants.RELATIONSHIP_EFFECTIVY_USAGE + "].torel.from.id");
		MapList gbomDVIdList = new MapList();
		gbomDVIdList = DomainRelationship.getInfo(context, (String[]) gbomRelIdList.toArray(gbomRelIDsArray),relSel);
		//Mapping GBOMRelIds to fetched gbomDVIdList
		HashMap gbomRelDVIdMap = new HashMap();
		for (int i = 0; i < gbomDVIdList.size(); i++){
			gbomRelDVIdMap.put(gbomRelIDsArray[i], gbomDVIdList.get(i));
		}
		
		boolean isLfDVGbomVariantMatching = true;
		if (evaluatedParts != null && evaluatedParts.size() > 0) {
			//We get the DV IDs mapped LF IDs from Map
			Map lfDVsInfoMap = (Map) lfDVMap.get(strFeatureId);
			StringList lfDVIdListFromMap = new StringList();
			Object objLFDV = lfDVsInfoMap.get("from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id");
			if(!ConfigurationUtil.isObjectNull(context, objLFDV)){
				if(objLFDV instanceof StringList){
					lfDVIdListFromMap = (StringList) lfDVsInfoMap.get("from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id");
				}else if(objLFDV instanceof String){
					lfDVIdListFromMap.add((String) lfDVsInfoMap.get("from[" + ConfigurationConstants.RELATIONSHIP_VARIES_BY + "].to.id"));
				}
			}		
			for (int i = 0; i < evaluatedParts.size(); i++) {
				//Get all the Effectivity Usage defined in GBOM of LF part in each iteration
				Map partMap = (Map) evaluatedParts.get(i);
				String gbomRelId = (String) partMap.get("id[connection]");
				Map gbomRelInfoMap  = (Map) gbomRelDVIdMap.get(gbomRelId);
				StringList gbomVariantIdList = new StringList();
				//We get the DV IDs mapped with GBOM Rel
				Object objSelDVVal = gbomRelInfoMap.get("frommid[" + ConfigurationConstants.RELATIONSHIP_EFFECTIVY_USAGE + "].torel.from.id");
				if(!ConfigurationUtil.isObjectNull(context, objSelDVVal)){
					if(objSelDVVal instanceof StringList){
						gbomVariantIdList = (StringList) gbomRelInfoMap.get("frommid[" + ConfigurationConstants.RELATIONSHIP_EFFECTIVY_USAGE + "].torel.from.id");
					}else if(objSelDVVal instanceof String){
						gbomVariantIdList.add((String) gbomRelInfoMap.get("frommid[" + ConfigurationConstants.RELATIONSHIP_EFFECTIVY_USAGE + "].torel.from.id"));
					}
				}
				isLfDVGbomVariantMatching = (gbomVariantIdList.containsAll(lfDVIdListFromMap));//If DV ID of LF is not having Effectivity Usage in GBOM (blank effectivity) then returns false
				if (!isLfDVGbomVariantMatching){//By default the value of isLfDVGbomVariantMatching is set true so in case of NO DVs this key won't get added to Map and part will be evaluated						
					partMap.put("notResolvedCompletelyKey", "true");
				}
			}
		}
			
		//Code added for GBOM Part Evaluation Ends here
		//------------------------------------------------------------
		//END of GBOM Part Evaluation Modification
		//------------------------------------------------------------
		...
		for(int j=0;j<evaluatedParts.size();j++){
			...
			if(!(gbom.get("notResolvedCompletelyKey") != null && gbom.get("notResolvedCompletelyKey").toString().equalsIgnoreCase("true"))){//This condition is added so that Duplicate Part which is notResolvedCompletely are not removed from evaluatedParts
...
public Vector renderActionIconsOnPreviewBOMForLeafLF(Context context, String[] args) throws FrameworkException {
    ...
    /**This method is used to get the Release Phase(Development/Production) attribute value of Part.
     * This method is called on initial loading of Preview EBOM Page.
     * 
     * @param context
     *        The ematrix context object
     * @param args
     *         String[]
     * @return StringList
     *         List of Release Phase (Development/Production)attribute value of Parts.
     * @throws
     *        Framework Exception
     * @since
     *        R419.HF6                
     */
    public static StringList getPartPhaseOnPreviewEBOM(Context context, String[] args)throws FrameworkException
    {
    	StringList listOfPartPhase = new StringList();
    	String strLanguage = context.getSession().getLanguage();
		try 
		{
			HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
			List<Map> objectList  = (List)parametersMap.get("objectList");
			for(Map LFObjMap : objectList)
			{
				if(LFObjMap != null && LFObjMap.size()> 0 && !"true".equalsIgnoreCase((String)LFObjMap.get("Root Node"))){
				   String strReleasePhase = (String)LFObjMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_RELEASE_PHASE);
				   String strRelePhase = "";
				   if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_DEVELOPMENT.equalsIgnoreCase(strReleasePhase)){
					   strRelePhase = EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Development",strLanguage);
				   }else if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_PRODUCTION.equalsIgnoreCase(strReleasePhase)){
					   strRelePhase = EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Production",strLanguage);
				   }
				   listOfPartPhase.add(strRelePhase); 
				}
				else if("true".equalsIgnoreCase((String)LFObjMap.get("Root Node")))
				{
					DomainObject domObj = new DomainObject((String)LFObjMap.get(DomainConstants.SELECT_ID));
					String strPCTLPReleasePhase = domObj.getInfo(context, "from["+ ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART +"].to.attribute["+ ConfigurationConstants.ATTRIBUTE_RELEASE_PHASE +"].value"); 
					if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_DEVELOPMENT.equalsIgnoreCase(strPCTLPReleasePhase)){
						listOfPartPhase.add(EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Development",strLanguage));
					}else if(ConfigurationConstants.RANGE_VALUE_RELEASE_PHASE_PRODUCTION.equalsIgnoreCase(strPCTLPReleasePhase)){
						listOfPartPhase.add(EnoviaResourceBundle.getProperty(context, "Configuration","emxConfiguration.ReleasePhase.Production",strLanguage));
					}else{
						listOfPartPhase.add(DomainConstants.EMPTY_STRING);
					}
				}
			}
		}
		...
		return listOfPartPhase;
    }
    
    /**This method is used to get the Change Controlled(Enabled;Any Change /Disabled / Change Id) attribute value of Part.
     * This method is called on initial loading of Preview EBOM Page.
     * 
     * @param context
     *        The ematrix context object
     ...
     *         String[]
     * @return StringList
     *         List of Change Controlled(True/False) attribute value of Parts.
     * @throws
     *        Framework Exception
     * @since
     *        R419.HF6                
     */
    public static StringList getChangeControlledOnPreviewEBOM(Context context, String[] args)throws FrameworkException
    {
    	StringList listChangeControlled = new StringList();
    	String strLanguage = context.getSession().getLanguage();
    	try
    	{
    		HashMap parametersMap  = (HashMap) JPO.unpackArgs(args);
			...
			{
				if(LFObjMap != null && LFObjMap.size()> 0 && !"true".equalsIgnoreCase((String)LFObjMap.get("Root Node"))){
				   String strChangeControlled = (String)LFObjMap.get(ConfigurationConstants.SELECT_ATTRIBUTE_CHANGE_CONTROLLED);
				   if(strChangeControlled.contains("content")){
					    if("CSV".equalsIgnoreCase(strExportFormat)){
					    	strChangeControlled = strChangeControlled.substring(strChangeControlled.indexOf("/>")+2, strChangeControlled.lastIndexOf("</a>"));
					    }else{
					    	strChangeControlled = strChangeControlled.replace("content", "popup");
					    }
				   }
				   listChangeControlled.add(strChangeControlled);
				}
				else if("true".equalsIgnoreCase((String)LFObjMap.get("Root Node")))
				{
					DomainObject domObj = new DomainObject((String)LFObjMap.get(DomainConstants.SELECT_ID));
					String strPCTLPart  = domObj.getInfo(context, "from["+ ConfigurationConstants.RELATIONSHIP_TOP_LEVEL_PART +"].to.id");
					if(ProductLineCommon.isNotNull(strPCTLPart)){
						HashMap partMap     = new HashMap();
						List<Map> objList   = new MapList();
						partMap.put(DomainConstants.SELECT_ID, strPCTLPart);
						objList.add(partMap);
						...
						if(lstChangeControlled.size() > 0){
							String strChangeControlled = (String)lstChangeControlled.get(0);
							if(strChangeControlled.contains("content")){
								if("CSV".equalsIgnoreCase(strExportFormat)){
							    	strChangeControlled = strChangeControlled.substring(strChangeControlled.indexOf("/>")+2, strChangeControlled.lastIndexOf("</a>"));
							    }else{
							...
							listChangeControlled.add(DomainConstants.EMPTY_STRING);
						}
					}else {
						listChangeControlled.add(DomainConstants.EMPTY_STRING);
					}
