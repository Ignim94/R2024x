	...
	private boolean processMFForRuleMigration( Context context)throws Exception{
        ...
        try
        {
            ...
            for (Iterator FLIdIttr = objectIds.iterator(); FLIdIttr.hasNext();) {
				...
				lstMultiValueList.add("to["+ConfigurationConstants.RELATIONSHIP_RIGHT_EXPRESSION+"].id");
				lstMultiValueList.add("to["+ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION+"].id");
				
				Map flDataMap = domFLObj.getInfo(context, objectSelect, lstMultiValueList);
