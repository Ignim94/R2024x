...
public class ManufacturingPlanBase_mxJPO extends emxDomainObject_mxJPO {
    ...
    public StringList getArchivedType(Context context, String[] args)throws Exception {
    	...
    	try {
            ...
            for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
            	...
            	typeList.add(((Map) iterator.next()).get(DomainConstants.SELECT_TYPE).toString());
    ...
    public StringList getArchivedRevision(Context context, String[] args)throws Exception {
    	...
    	try {
            ...
            for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
            	...
            	revisionList.add(((Map) iterator.next()).get(DomainConstants.SELECT_REVISION).toString());
    	...
    	try {
            ...
            for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
            	...
            	if((tempMap).containsKey(DomainConstants.SELECT_DESCRIPTION)){
            		descList.add(tempMap.get(DomainConstants.SELECT_DESCRIPTION).toString());
            	}else{
    	...
    	try {
            ...
            for (Iterator iterator = objectList.iterator(); iterator.hasNext();) {
            	...
            	if((tempMap).containsKey(DomainConstants.SELECT_OWNER)){
            		ownerList.add(tempMap.get(DomainConstants.SELECT_OWNER).toString());
            	}else{
	...
	public String createManufacturingPlan(Context context, String[] args)
		...
		try {
            ...
            //if Mfg Plan Modeler exists, call its API to compute and stamp PrecomputedCfgFilter on the newly created Mfg Plan
            if(null!=strObjectId && !strObjectId.isEmpty() && null!=strPlannedForID && !strPlannedForID.isEmpty()){
                Class<?> mfgPlanMdlClass = Class.forName( "com.dassault_systemes.enovia.manufacturingplan.modeler.ManufacturingPlan" );
                if(null!=mfgPlanMdlClass){
                    Constructor constructor = mfgPlanMdlClass.getConstructor(String.class);
                    Object mfgPlanObj = constructor.newInstance(strObjectId);
                    
                    Method initPrecompFltBinMethod = mfgPlanObj.getClass().getMethod("initializePrecomputedFilterBinary", Context.class);
                    if(null!=initPrecompFltBinMethod){
                        initPrecompFltBinMethod.invoke(mfgPlanObj, context);
                    }
                }
            }
    ...
    /**
     ...
     {
    	 matrix.util.StringList manRevList = new StringList();
     ...
     {
         matrix.util.StringList manPlanList = new StringList();
         try{
        	 ManufacturingPlan manPlan = new ManufacturingPlan();
             HashMap programMap = (HashMap) JPO.unpackArgs(args);
             MapList objectList = (MapList) programMap.get("objectList");
             ...
             for (int i = 0; i < objectList.size(); i++)
             {
            	 ...
            	 MapList ftrList = new MapList(10);
            	 String mPlanTitle ="";
...
public static String getTypeIconProperty(Context context, String type)
	...
	public int deleteConnectedManufacturingPlans(Context context, String args[])throws Exception{
		...
		try {
		    ...
		    lstMultiValueList.add(managedSeriesSelect);
		    
		    relSelects.add(DomainConstants.SELECT_ID);
		    relSelects.add(DomainConstants.SELECT_TYPE);
		    ...
		    if(strMPM!=null && !strMPM.equals("")){
		    	...
		    	relIds = domMPM.getInfo(context,relSelects, lstMultiValueList);
		    }
    ...
    /*
    * Checks if this is the leaf node
    */
	private boolean isLeaf(Context context, String objectID)
			throws FrameworkException {
        
		boolean isLeaf =true;
		String derivedAbstractToSelectable = "from[" + PropertyUtil.getSchemaProperty(context,"relationship_DERIVED_ABSTRACT") + "].id";
		String derivedToSelectable = "from[" + PropertyUtil.getSchemaProperty(context,"relationship_Derived") + "].id";
		String mainDerivedToSelectable = "from[" + PropertyUtil.getSchemaProperty(context,"relationship_MainDerived") + "].id";
		try {
			Map objectInfo = (new DomainObject(objectID)).getInfo(context, new StringList(derivedAbstractToSelectable));
			if (objectInfo.containsKey(derivedToSelectable) || objectInfo.containsKey(mainDerivedToSelectable)) {
				isLeaf=false;
			}
	...
	public int deleteManufacturingPlanMaster(Context context, String args[])
		...
		try {
            ...
            if(!isLeaf(context, strManufacturingPlanId)){
                iResult = 1;
                String language = context.getSession().getLanguage();
                String strAlertMessage = EnoviaResourceBundle.getProperty(context,SUITE_KEY,
                "DMCPlanning.Delete.NonLastNode",language);
                throw new FrameworkException(strAlertMessage);
            }
										...
										if(!prdMapList.isEmpty()){
											for(int j=0; j<prdMapList.size();j++){
												...
												if(tempMpImplements.contains(strPrd) && prdsLF.contains(strPrd)){
													...
													/*${CLASS:ManufacturingPlanSearchBase} mpSearch=new ${CLASS:ManufacturingPlanSearchBase}(context,jpoArgs );
													...
													StringList strList = (StringList) JPO.invoke(context, "ManufacturingPlanSearchBase", jpoArgs, "getMPofCurrentRevision", jpoArgs, StringList.class);
													StringList strListRetro = (StringList) JPO.invoke(context, "ManufacturingPlanSearchBase", jpoArgs, "getMPofCurrentAndAllPreviousRevisions", jpoArgs, StringList.class);
...
//									}
									...
									while(revItr.hasNext()){
										...
										lowerRevIdList.add(revMap.get(SELECT_ID).toString());
    ...
    public int CheckIfCompliant(Context context, String args[])throws Exception{
		int iResult = 0;
		try{
			if(!com.dassault_systemes.enovia.governance.modeler.GovernanceModelerServices.isCompositionMatrixEnabled(context)) {
				return 0;
			}
                    		...
                    		String prdSiblId="";
                    		  while(prdItr.hasNext())
                    		  {
								...
								if (mpImplements.contains(prdSiblId)) {
									...
									mp.put("objectId", prdSiblId);
									String[] jpoArgs = JPO.packArgs(mp);
									/*${CLASS:ManufacturingPlanSearchBase} mpSearch = new ${CLASS:ManufacturingPlanSearchBase}(
											context, jpoArgs);
				...
				//if(strMPIntent.equalsIgnoreCase(ManufacturingPlanConstants.RANGE_VALUE_RETROFIT)){
					if(strType!=null && mxType.isOfParentType(context, strType,ManufacturingPlanConstants.TYPE_PRODUCTS)){
						if(null != strProductStructureMPId && !"null".equals(strProductStructureMPId) && !"".equals(strProductStructureMPId)){
							...
							slObjectSelcts.addElement(DomainConstants.SELECT_ID);
							slObjectSelcts.addElement(DomainConstants.SELECT_TYPE);
							
							StringList lstMultiValueList = new StringList();
							lstMultiValueList.add(SELECT_MPI_FROM);
							lstMultiValueList.add(SELECT_CONTEXT_PRODUCT);
							Map  mapContext = dmoProduct.getInfo(context, slObjectSelcts, lstMultiValueList);
...
//							StringList slProduct =  dmoParentPRoduct.getInfoList(context, SELECT_MANAGED_FEATURES);
							...
							slObjSels.addElement(DomainConstants.SELECT_ID);
							slObjSels.addElement(DomainConstants.SELECT_TYPE);
							
							StringList lstMultiValueList2 = new StringList();
							lstMultiValueList2.add(SELECT_CONNECTED_MANUFACTURING_PLAN);
							lstMultiValueList2.add(SELECT_CONNECTED_MANUFACTURING_PLAN_NAME);
							MapList mlProductsMPs=DomainObject.getInfo(context, strArrPrdIds, slObjSels, lstMultiValueList2);
...
//								String strFeatureType = dmoProductRev.getInfo(context, SELECT_TYPE);
								StringList slManufPlans=new StringList();
								...
								StringList slManufPlanNames= new StringList();
	...
	public int disconnectManufacturingPlanOfProduct(Context context,
		...
		try {
			...
			mlProductMPs = domobj.getRelatedObjects(context, stbRelSelect
					...
					0, null, null, null);
			StringList sListMPIds = new StringList();
	...
	public boolean CheckAssociatedPlan(Context context, String args[])throws Exception{	
		...
		try {
			...
			Map mapParentProductId = domParentObj.getInfo(context, new StringList(SELECT_PARENT_PRODUCT));
			String strParentProductId="";
			if(mapParentProductId.containsKey(SELECT_PARENT_PRODUCT) )
				strParentProductId=(String)mapParentProductId.get(SELECT_PARENT_PRODUCT);
			else if(mapParentProductId.containsKey(SELECT_PARENT_MAIN_PRODUCT) )
				strParentProductId=(String)mapParentProductId.get(SELECT_PARENT_MAIN_PRODUCT);
			Model modelBean = new Model(strParentProductId);
			MapList mlMasterCompositionToModel = modelBean.getMasterComposition(context,(short)0,"");

			
			
			if(mlMasterCompositionToModel.size()>0 || modelIds.size()>0){// Master Composition connected to context Model OR Product has Product in it.
				...
				while(modelIter.hasNext()){
					...
					StringList slMps= new StringList();
	...
	public StringList CheckAssociatedPlanOnPrd(Context context, String args[])throws Exception{	
		...
		result.add(String.valueOf(innerFlag));
	...
	public List getDerivationType(Context context, String args[])
		...
		StringList derivationTypeList = new StringList();
	...
	public MapList getEffManufacturingPlanDerivationChain(Context context,
		...
		try {
			...
			/*${CLASS:CFPModel} cfpModelJpo = new ${CLASS:CFPModel}(context, null);*/
		...
		if(domContextObj.isKindOf(context,ManufacturingPlanConstants.TYPE_MODEL)){
			if(recurseLevel == 1){//get only Main Derived derivations - Revisions
				...
				mlDerivationStructure = (MapList) JPO.invoke(context, "CFPModel", null, "getManufacturingPlanRevision", args, MapList.class);
			}
		else{
			...
			MapList mainDervML = (MapList) JPO.invoke(context, "CFPModel", null, "getManufacturingPlanRevision", args, MapList.class);
		...
		for(int i=0;i<mlDerivationStructure.size();i++){
					 	...
					 	DomainObject domObj = new DomainObject(objectId);
					 	//disable selection of objects or parent objects are of ManufacturingPlan type in frozen state          
					 	if(ManufacturingPlan.isFrozenState(context, objectId)){
     ...
     */
 	...
 	private HashMap<String, HashMap<String, StringList>> getMPsThroughMasterModels(	Context context, HashMap<String, String> idParentMap) throws Exception {
		...
		for (String modelSelectable : modelSelectables) {
			lstMultiValueList.add(modelSelectable);
		}
		
		MapList modelsInfo = DomainObject.getInfo(context, parentIdsArr, selectables, lstMultiValueList);
		...
		StringList slMultiValueList = new StringList();
		slMultiValueList.add(mpSelectable);
		slMultiValueList.add(mpSelectable2);
		
		MapList mpsInfo = DomainObject.getInfo(context, modelIdsArr, mpSelectables, slMultiValueList);
				...
				if(relMapList!=null && relMapList.size()>0){
					...
					if(ProductLineCommon.isKindOfRel(context, (String)relMap.get(SELECT_RELATIONSHIP_TYPE), ManufacturingPlanConstants.RELATIONSHIP_ASSOCIATED_MANUFACTURING_PLAN)||
						...
						strParentId = (String)relMap.get(SELECT_FROM_ID);
							...
							ProductLineCommon.isKindOfRel(context,  (String)relMap.get(SELECT_RELATIONSHIP_TYPE), ManufacturingPlanConstants.RELATIONSHIP_DERIVED_ABSTRACT)){
