    	  ...
    	  try{
    		  ...
    		  if(ruleComplexity.trim().equals("Complex")){
    			  ...
    			  for(int k = 0; k < slParentPhysicalId.size(); k++){
    				  ...
    				  jsonObjBuild.add("contextId", GLOBAL_MODEL_PHYSICAL_ID); //physicalid of the Model context
    				  jsonObjBuild.add("parentId", slParentPhysicalId.get(k)); //physicalid of the CF 
    				  jsonObjBuild.add("objId", slChildPhysicalId.get(k)); //physicalid of the CO
    				  jsonObjBuild.add("relId", slRelCOPhysicalId.get(k)); //physicalid of the CO rel
    				  jsonObjBuild.add("insertAsRange", false);
    				  JsonObject effObj = jsonObjBuild.build();
