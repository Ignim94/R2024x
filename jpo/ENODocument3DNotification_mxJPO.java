...
import com.matrixone.enovia.bps.notifications.NotificationRegisterService;
import com.matrixone.apps.domain.DomainConstants;
import com.matrixone.apps.domain.DomainObject;
import com.matrixone.apps.domain.util.ENX3DNotification;
...
public class ENODocument3DNotification_mxJPO {
	...
	private void  sendNotification(Context context,NotificationData notificationData)throws Exception {
		
		if(!isAppRegistered || UIUtil.isNullOrEmpty(notificationJSON)) {			
			NotificationRegisterService.deployNotificationSettings(context,notificationData.getAppID(),notificationData.getNotificationNLSKeyPrefix(),notificationData.getNotificationStringResourceFile());
			isAppRegistered = true;				
			notificationJSON = ENX3DNotification.getSettingsAndNLSObject(context,docMgmtAppKey, DOCUMENT_NOTIFICATION_KEYPREFIX, DOCUMENT_STRING_RESOURCE_FILE);
		}
		
		
		ENX3DNotification notificationObj = new ENX3DNotification(context, NotificationUtil.create3DNotificationMessageJSON(context,notificationData) , new NotificationService(context).get3DNotificationsURL()  , 10000, docMgmtAppKey,"","" );
				
		notificationObj.setstrAppNotificationSettings(notificationJSON);
		notificationObj.registerNotification(context);
	}
