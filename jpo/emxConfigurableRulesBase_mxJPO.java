  ...
  protected static final String COMPLEX_INCLUSION_RULE_KEY = "emxProduct.RuleCreation.ComplexInclusion";
    ...
    **/
    ...
    public static List getExpression (Context context, String[] args) throws Exception{
        ...
        StringList lstExpressionList = new StringList();
...
/**
	...
	throws Exception {
		StringList lstExpressionList = new StringList();
		StringList finalExpressionList= new StringList();
		try {
			HashMap parametersMap = (HashMap) JPO.unpackArgs(args);
			List lstobjectList = (MapList) parametersMap.get("objectList");
        ...
        throws Exception
    {
        ...
        for (int j=0; j<lstobjectList.size(); j++)
        {
            ...
            if(ProductLineCommon.isOfParentRel(context,strRelType,ConfigurationConstants.RELATIONSHIP_LOGICAL_STRUCTURES)
            		...
            		|| ProductLineCommon.isOfParentRel(context,strRelType,ConfigurationConstants.RELATIONSHIP_CONFIGURATION_OPTIONS)
  ...
  */
	...
	public Vector showInclusionRuleLink(Context context, String[] args)
		...
		for (int i = 0; i < lstobjectList.size(); i++) {
			...
			.get("tomid["+ ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION+ "].from["+ConfigurationConstants.TYPE_INCLUSION_RULE+"].type");
			...
			if(iRuleIndex!=-1){
				...
				.get("tomid["+ ConfigurationConstants.RELATIONSHIP_LEFT_EXPRESSION+ "].from["+ConfigurationConstants.TYPE_INCLUSION_RULE+"].id");
 ...
 throws Exception {
		...
		String[] relIdsArray = new String[lstobjectList.size()];
		for (int i = 0; i < lstobjectList.size(); i++) {
			// Get the Relationship IDs
			relIdsArray[i] = (String) ((Map) lstobjectList.get(i)).get(DomainRelationship.SELECT_ID);
		}

		MapList relMapList = DomainRelationship.getInfo(context, relIdsArray, slRelSelects);
		HashMap mapRelIRId = new HashMap();
		for(Object obj: relMapList)
		{
			Map map = (Map)obj;
			mapRelIRId.put(map.get(DomainRelationship.SELECT_ID), map.get("tomid["+ConfigurationConstants.ATTRIBUTE_LEFT_EXPRESSION+"].from["+ConfigurationConstants.TYPE_INCLUSION_RULE+"].id"));
		}
		...
		for (int i = 0; i < lstobjectList.size(); i++) {
			...
			strLevel = (String)objectMap.get("id[level]");
			String strIncRuleID = "";
			strIncRuleID = (String)mapRelIRId.get(strRelId);//(String) objectMap.get("tomid[Left Expression].from[Inclusion Rule].id");
