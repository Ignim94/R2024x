	 ...
	 /**
		...
		public void connectConfigurationOption(Context context, String[] args)throws FrameworkException {
				...
				try{
				    ...
				    StringList lstVBOChildTypes = ProductLineUtil.getChildTypesIncludingSelf(context,ConfigurationConstants.TYPE_VARIABILITYOPTION);
				    if(strConfigurationFeatureOIDs != null && strConfigurationFeatureOIDs[0]!= null && !strConfigurationFeatureOIDs[0].equals("")&& !strConfigurationFeatureOIDs[0].equals("null")){
						...
						String toObjectType = toDomObj.getInfo(context, DomainConstants.SELECT_TYPE);
						...
						if(bInvalidState == true){
							...
							if(lstVariantValueChildTypes.contains(toObjectType)){
								DomainRelationship.connect(context, fromDomObj, ConfigurationConstants.RELATIONSHIP_VARIANT_VALUES, toDomObj);
								System.out.println("Connection Done !!! with RELATIONSHIP_VARIANT_VALUES");
							}else if(lstVBOChildTypes.contains(toObjectType)){
								DomainRelationship.connect(context, fromDomObj, ConfigurationConstants.RELATIONSHIP_GROUPED_VARIABILITY_CRITERIA, toDomObj);
								System.out.println("Connection Done !!! with RELATIONSHIP_GROUPED_VARIABILITY_CRITERIA");
							}else{
							DomainRelationship.connect(context, fromDomObj, ConfigurationConstants.RELATIONSHIP_CONFIGURATION_OPTIONS, toDomObj);
								System.out.println("Connection Done !!! with RELATIONSHIP_CONFIGURATION_OPTIONS");
							}
							...
							String[] strarrayparentOID = (String[])requestMap.get("parentOID");
								if(strarrayparentOID != null){
									...
									if(strParentType != null && !"".equals(strParentType) &&  mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_GENERAL_CLASS)){
										...
										DomainRelationship.connect(context, strParentID, ConfigurationConstants.RELATIONSHIP_CLASSIFIED_ITEM, newObjID,false);
										System.out.println("Connection Done !!! with Classified Item rel");
									}
								}
						}
				    }else{
						...
						if(strarrayparentOID != null){
							...
							if(strParentType != null && !"".equals(strParentType) &&  mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_MODEL)){
								Model confModel = new Model(strParentID);
								confModel.connectCandidateConfigurationFeature(context, newObjID);
								System.out.println("Connection done with Candidate rel");
							}
							else if(strParentType != null && !"".equals(strParentType) && mxType.isOfParentType(context,strParentType,ConfigurationConstants.TYPE_GENERAL_CLASS)){
...
/**
				...
				public static int inheritMandatoryConfigurationFeatures(Context context, String []args) throws FrameworkException{
					...
					try{
						...
						ConfigurationFeature.copyMandatoryConfigurationFeature(context, fromObjectID, toObjList, toObejctId);
							  }
						  }						  
					}
					}
...
//							context, (Object) connectIdMap.get(coIR));
		    ...
		    for (int j = 0; j < lstObjectIdsList.size(); j++) {
		        ...
		        if(ProductLineCommon.isNotNull(strRelationship)) {
		        	if (strRelationship.equals(ConfigurationConstants.RELATIONSHIP_MANDATORY_CONFIGURATION_FEATURES)) {
		        		...
		        		}else if(ConfigurationConstants.RANGE_VALUE_SYSTEM.equalsIgnoreCase(strConfType) || ConfigurationConstants.RANGE_VALUE_MANDATORY.equalsIgnoreCase(strConfType)){
	...
	public StringList parentConfigurationFeatureName(Context context, String []args) throws FrameworkException{
		...
		try{
			...
			StringList lstMultiValueList = new StringList();
			lstMultiValueList.add("to["+ConfigurationConstants.RELATIONSHIP_CONFIGURATION_FEATURES+"].from["+ConfigurationConstants.TYPE_CONFIGURATION_FEATURE+"].name");
			lstMultiValueList.add("to["+ConfigurationConstants.RELATIONSHIP_CONFIGURATION_OPTIONS+"].from["+ConfigurationConstants.TYPE_CONFIGURATION_FEATURE+"].name");
			...
			MapList mpCFParentDetails = DomainObject.getInfo(context, strCFIDs, selectList, lstMultiValueList);
...
*/
	...
	public int checkIfCFStructureUsedInFOEffectivity(Context context,
		...
		int iReturn = 0;
		try {
		DomainObject domFrom= new DomainObject(_sToSideObjectId);
		if(UIUtil.isNotNullAndNotEmpty(_sToSideObjectId) && domFrom.exists(context) && domFrom.isKindOf(context, ConfigurationConstants.TYPE_PRODUCT_LINE)){
			return 0;
		}
		...
		public static StringList getRDO(Context context, String[] args) throws FrameworkException{
			try{
				...
				DomainObject domObj=null;
		 	    ...
		 	    objectSelects.add("organization.property[Title].value");
			    StringList returnStringList = new StringList (objectMap.size());
				for (int i = 0; i < objectMap.size(); i++) {
					String strRDO = "";	
					Map outerMap = new HashMap();
					outerMap = (Map)objectMap.get(i);							
					strRDO=(String)outerMap.get("organization.property[Title].value");
		            if(strRDO == null || "".equals(strRDO)) {
					...
					if(isRootNode!=null && isRootNode.equalsIgnoreCase("true") ){
			            ...
			            strRDO=(String)rootMap.get("organization.property[Title].value");
			            if(strRDO == null || "".equals(strRDO)) {
		    ...
		    */
			...
			public MapList getVariabilityStructureForPLContext(Context context, String[] args)	
				...
				try{	
					...
					slObjSelects.addElement(ConfigurationConstants.SELECT_ATTRIBUTE_VARIANT_DISPLAY_TEXT);
					slObjSelects.addElement("altowner1");
					slObjSelects.addElement("organization.property[Title].value");
					StringList slRelSelects = new StringList();
					slRelSelects.addElement(ConfigurationConstants.SELECT_RELATIONSHIP_ID);
					slRelSelects.addElement(ConfigurationConstants.SELECT_RELATIONSHIP_TYPE);
					slRelSelects.addElement(ConfigurationConstants.SELECT_RELATIONSHIP_NAME);                     
			   ...
			   @com.matrixone.apps.framework.ui.ProgramCallable
				...
				// System.out.println("${CLASSNAME}.getConfigurationFeatureStructureForProductContext()....%%%%%%%%%% ......");
					...
					try{
					  	...
					  	strEmxTableRowId = (String) programMap.get("strSelectedTableIdsInSession");
					  	
					  	System.out.println("-------------------------------------------->In getVariabilityStructureAllocationMatixProductContext: ");
					  	String strvariabilityMode = (String) programMap.get("variabilityMode");
				         StringList slObjSelects = new StringList();
				         //Object Select
				         slObjSelects.addElement(ConfigurationConstants.SELECT_ID);
				         ...
				         relSelects.add("from.attribute["+ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME+"]");
				         ...
				         if(strEmxTableRowId != null && !"".equals(strEmxTableRowId)) {
			   ...
			   @com.matrixone.apps.framework.ui.ProgramCallable
				...
				public StringList getFeatureColumnForAllocationMatrix(Context context, String[] args)
				   ...
				   try {
					   ...
					   Map requestMap = (HashMap) programMap.get("requestMap");
					   MapList objectMapList = (MapList) programMap.get("objectList");
					   String strFeatureDisplayName = new String();
					   for(int i=0;i<objectMapList.size();i++){
						   Map objMap = (Map) objectMapList.get(i);
						   if(objMap.containsKey("from.attribute["+ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME+"]") && 
								   ProductLineCommon.isNotNull((String)objMap.get("from.attribute["+ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME+"]"))){
							   strFeatureDisplayName = (String) objMap.get("from.attribute["+ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME+"]");
							   returnList.add(strFeatureDisplayName);
						   }else{
							   //Fetch Parent name
							   ...
							   strFeatureDisplayName = domObj.getInfo(context, "attribute["+ConfigurationConstants.ATTRIBUTE_DISPLAY_NAME+"]");
        					   if(ProductLineCommon.isNotNull(strFeatureDisplayName)){
         						returnList.add(strFeatureDisplayName);         						
        					   }
						   }
					   }
