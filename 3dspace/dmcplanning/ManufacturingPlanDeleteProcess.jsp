    ...
    if(isBreakDownMPSelected){
     ...
     <%
    	...
    	try{
           ...
           if(errMsg.contains("java.lang.Exception: ")){
        	   errMsg = errMsg.replace("java.lang.Exception: ","");
        	   int msgStart = errMsg.indexOf("Message:");
        	   int msgEnd = errMsg.indexOf("Severity:");
        	   if((msgStart != -1) && (msgEnd != -1))
 	           {
 	        	   errMsg  = errMsg.substring(msgStart+8,msgEnd).trim();
 	           }
           }
           msg = errMsg;
         }
