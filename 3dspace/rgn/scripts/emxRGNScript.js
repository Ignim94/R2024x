...
function rgnFullSearch(table, type, policy, cancelNLS, field, invalidChars, cardinality) {
	const pattern = document.editForm[field + 'Display'].value.trim();
	var validationPattern = /[[:@<>%,\[\]]/;
	if (pattern && pattern.match(validationPattern))
		alert((invalidChars ? invalidChars : 'Invalid Characters'));
	else {
		require(['DS/RgnPreviewSearch/InputUI'], function (inputUI) {
			const searchPattern = inputUI.buildSearchPattern(pattern);
			var search = '../common/emxFullSearch.jsp?table='
				...
				+ (searchPattern ? ':=' + encodeHREF(searchPattern.replace("\"", "")).replace('|', ',') : '')
