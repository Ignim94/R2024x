...
function saveLibraries() {
    ...
    doPostRequest('../resources/rgn/ots', JSON.stringify(requestData), answerAnalysis)
}
...
function otsAcceptDrag(event) {
	if (event.preventDefault) {
        event.preventDefault();
    }
    event.target.classList.add('ots-drag-over');
    event.dataTransfer.dropEffect = 'copy';
    return false;
}

function otsGetDroppedData(event) {
	 event.preventDefault();
     var dropZone = event.target, 
     	 file = event.dataTransfer.files[0],
         reader = new FileReader(),
         addLibraries = function (otsLib) {
             var fileList = dropZone.querySelector('#fileList');
             otsLib.forEach(function (lib) {
            	 newOtsLibrary(lib.name, lib.code);
             });
         };
     reader.onload = function (loadEvent) {
         if (loadEvent.target.result.startsWith('[')) {
             addLibraries(JSON.parse(loadEvent.target.result));
             checkLibraries();
             editLibrary();
         } else {
        	 dropZone.ownerDocument.querySelector('.invalid-drop-format').classList.toggle('ots-hidden');
         }
		 dropZone.classList.remove('ots-drag-over')
     };
     reader.readAsText(file);
     return false;
}

function otsClearDrag(event) {
	event.target.classList.remove('ots-drag-over');
}
...
function initFileList(fileMap, doSave) {
    var name,
        ...
        fileList = document.getElementById('fileList'),
        dropZone = document.querySelector('.ots-drop-zone');
    for (name in map) {
    ...
    dropZone.ondragover = otsAcceptDrag;
    dropZone.ondragenter = otsAcceptDrag;
    dropZone.ondrop = otsGetDroppedData;
    dropZone.ondragleave = otsClearDrag;
...
function newOtsLibrary(name, code) {
    ...
    enableElements(['checkLibrary', 'methodPattern']);
    check.setAttribute('type', 'checkbox');
    check.onchange = updateSelection;
    setOtsLibraryName(span, name ? name : ('OTScript' + (indice + 1)));
    input.setAttribute('type', 'text');
    input.setAttribute('class', 'ots-hidden ots-lib-name-edition');
...
function enableElements(eltIds) {
    for (var i = 0; i < eltIds.length; i++) {
        document.getElementById(eltIds[i]).removeAttribute('disabled');
    }
}
...
function disableElements(eltIds) {
    for (var i = 0; i < eltIds.length; i++) {
        document.getElementById(eltIds[i]).setAttribute('disabled', 'true');
    }
}
...
function updateSelection(event) {
    var exportBtn = document.getElementById('exportSelection'),
        checkedFiles = document.getElementById('fileList').querySelectorAll('input:checked');
    if (checkedFiles.length === 0) {
        disableElement(exportBtn);
    } else {
        enableElement(exportBtn);
    }
}
...
function exportSelection() {
    var exportBtn = document.getElementById('exportSelection'),
    	checkedFiles = Array.from(document.getElementById('fileList').querySelectorAll('input:checked')).map(function (sel) {
    		return sel.nextElementSibling.id;
    	}),
    	otsDataList = getOTScriptData().filter(function (otsData) {
    		return checkedFiles.includes(otsData.name);
    	});
    saveData(otsDataList, 'export.json');
}
...
function deleteOtsLibrary(event) {
    ...
    sel.querySelector('input[type=checkbox]').classList.toggle('ots-hidden');
