...
<%
	...
	TemplateParameter[] params = CATRgnReportServices.getTemplateRepresentation(reqInfo, reportModelId).getParameters();
	...
	hiddenFields.put("configId", configId);

	String filterView = emxGetParameter(request,"filterView");
	hiddenFields.put("filterView", filterView);
	...
	for(int i = 0; i < nbInputs; i++) {
		...
		inputTypesInitValues.put(fieldName, value == null ? value : value.replaceAll("&lt;", "<")); //For display, get back the html tag lke opening char
	}
...
<BODY class="slide-in-panel" style="overflow-y: auto !important;" onload="rgnUpdateCreateParamForm(<%=XSSUtil.encodeForJavaScript(context,  String.valueOf(nbInputs) )%>);">
				...
				<% } else { 
					for(int i = 0; i< nbInputs; i++) {
						...
						boolean isMQLSearchType = isRelationshipType 
													|| (RgnConstants.RGN_BUNDLE.containsKey(RgnConstants.MQL_SEARCH_TYPES)
															&& Arrays.asList(RgnConstants.RGN_BUNDLE.getString(RgnConstants.MQL_SEARCH_TYPES).split("\\s*,\\s*")).contains(typeName));
