...
<%@ page import = "com.dassault_systemes.catrgn.reportNav.services.jpo.ObjectDataContainer" %>
<%@ page import = "com.dassault_systemes.catrgn.reportNav.services.jpo.BusObject" %>
<%@ page import = "com.dassault_systemes.catrgn.reportNav.services.jpo.VersionObject" %>
<%@ page import = "com.dassault_systemes.catrgn.services.util.misc.MQLServices" %>
...
<%
   ...
   String outputFormatRevisionLabel = I18nUtil.STR_REVISION.get(i18nCtx);
   String outputFormatTypeLabel = I18nUtil.STR_TYPE.get(i18nCtx);
...
//		throw new RuntimeException("invalid parameter: objectId");
   ...
   String outputFormatRevision = null;
   String outputFormatType = null;
   String outputFormatTitle = emxGetParameter(request, "outputFormatTitleField");
   ...
   String[] outputFormatPolicies = MQLServices.mqlPrintTypeSelects(context, "Report Template", "policy");
   String[] outputFormatPoliciesHidden = MQLServices.mqlPrintTypeSelects(context, "Report Template", "policy.hidden");
   ...
   <%		try {
			if (isEditMode) {
				...
				String libraryFileName = emxGetParameter(request, "outputFormatFileName");
				objectId = services.createOutputFormat(emxGetParameter(request, "outputFormatNameField"),
   									emxGetParameter(request, "outputFormatTitleField"),
   									emxGetParameter(request, "outputFormatDescriptionField"),
   									libraryFileName.equals("none") ? null : libraryFileName,
   									emxGetParameter(request, "outputFormatPolicy"));
			}
   		...
   		if(isEditMode && objectId != null) {
   			...
   			if (role != null && (!role.equals("null") || !role.equals("")|| !role.equals(" "))) {
				try {
					ObjectDataContainer ofContainer = new ObjectDataContainer(services.requestInfo, objectId);
					BusObject of = ofContainer.getBus(objectId);
					outputFormatName = of.getName();
					outputFormatTitle = of.getTitle();
					outputFormatRevision = of.getRevision();
					outputFormatType = of.getType(i18nCtx);
					outputFormatDescription = of.getDescription();
					currentOFPolicy = of.getPolicy(i18nCtx);
					if (of instanceof VersionObject)
						currentOFFileName = ((VersionObject)of).getFirstFileName();
				} catch (Exception e) {
   					e.printStackTrace();
					reportError = e.getMessage();
				}
...
<BODY OnLoad="takeFocus();" class="slide-in-panel" style="overflow-y: auto !important;">
			...
			<tbody>
				...
				<tr>
					<td class="label" width="150" align="left">
						<%=XSSUtil.encodeForHTML(context, outputFormatRevisionLabel)%>
					</td>
				</tr>
				<tr>
					<td  class="inputField">
						<% if(outputFormatRevision != null){ %><%=XSSUtil.encodeForHTML(context, outputFormatRevision)%><% }; %>
					</td>
				</tr>
				<!--Type Field -->
				<tr>
					<td class="label" width="150" align="left">
						<%=XSSUtil.encodeForHTML(context, outputFormatTypeLabel)%>
					</td>
				</tr>
				<tr>
					<td  class="inputField">
						<% if(outputFormatType != null){ %><%=XSSUtil.encodeForHTML(context, outputFormatType)%><% }; %>
					</td>
				</tr>
				<% } %>
				<!--Title Field -->
				...
				<tr>
					<td  class="field">
						<input type="text" id="outputFormatTitleField" name="outputFormatTitleField" size="20" value="<% if(outputFormatTitle != null){ %><%=XSSUtil.encodeForHTMLAttribute(context, outputFormatTitle)%><% }; %>"/>
					</td>
				</tr>
				<!--Description Field -->
				<tr>
