  ...
  try{ 
    ...
    String strTargetObjId = (String)reqMap.get("objectId");//destination obj
     ...
     if(reqMap.get("share").toString().equals("on")){       
         ...
         //Loop getinfo call just to fetch type of selected Objects to be copied - can be optimized also get selection type which can be put into map
         //-------------------------------Code Added For Fetching objectType and Selection Type of featureToShare STARTS-------------------------------//
         String[] featureToShareIDsArray = new String[selectedFeatureList.size()];
         for(int iCount=0; iCount < selectedFeatureList.size(); iCount++){
        	 HashMap featureMap = (HashMap)selectedFeatureList.get(iCount);
        	 String featureToShareID = featureMap.get("OId").toString();
        	 //IF
        	 if(featureToShareID.contains(",")){
        		 StringTokenizer strSelectedObjectTokenizer = new StringTokenizer(featureToShareID,",");
            	 for(int j = 0; j < strSelectedObjectTokenizer.countTokens(); j++){
            		 featureToShareID = strSelectedObjectTokenizer.nextElement().toString();
                     break;
                     }
        	 }        
        	 featureToShareIDsArray[iCount] = featureToShareID;
         }
         StringList relSelectable = new StringList(ConfigurationConstants.SELECT_TYPE);
 		 relSelectable.add("attribute["+ConfigurationConstants.ATTRIBUTE_LOGICAL_SELECTION_TYPE+"]");
		 MapList featureToShareSelectionTypeList = DomainObject.getInfo(context, featureToShareIDsArray, relSelectable);
		 //-------------------------------Code Added For Fetching objectType and Selection Type of featureToShare ENDS-------------------------------//
         for(int iCount=0; iCount < selectedFeatureList.size(); iCount++){
        	 HashMap featureMap = (HashMap)selectedFeatureList.get(iCount);
        	 relType = "relationship_LogicalFeatures";
        	 featureToShare = featureMap.get("OId").toString();
             ...
             strFeatureType = (String) ((Map) featureToShareSelectionTypeList.get(iCount)).get(ConfigurationConstants.SELECT_TYPE);
             StringTokenizer strSelectedObjectTokenizer = new StringTokenizer(featureToShare,",");
             String strFeatureSelectionType = (String) ((Map) featureToShareSelectionTypeList.get(iCount)).get("attribute["+ConfigurationConstants.ATTRIBUTE_LOGICAL_SELECTION_TYPE+"]");
             for(int j = 0; j < strSelectedObjectTokenizer.countTokens(); j++){
                 strSelectedObject = strSelectedObjectTokenizer.nextElement().toString();
...
//             else{
                 ...
                 //Code added for calling getXMLForSBCreateForMultipleFeatures
                 //For share relType is appended with strSourceRelId and fore clone we will append strPartFamilyDisplay
                 relType = relType + "|" + strSourceRelId + "|" + strFeatureSelectionType;               
                 mpSelectedFeatures.put(strSelectedObject, relType);
                 mpSelectedFeatures.put("isFromLFShare", Boolean.TRUE);                		
         ...
         if(flagValidationFailed == false){
             if(!listSubFeats.contains(featureToShare)){
            	 xml = LogicalFeature.getXMLForSBCreateForMultipleFeatures(context, paramMap, mpSelectedFeatures, strObjectIdList);
            	 %>
                 <script language="javascript" type="text/javaScript">
         ...
         <% 
            ...
            for(int iCount=0; iCount< selectedFeatureList.size(); iCount++){
                ...
                strFeatureLevelId = (String)featureMap.get("LevelId");
                ...
                if(!connectedOIDMap.containsKey(strFeatureLevelId)){
                    ...
                    parentOID = parentClonedObject.getId(context);
                    ...
                    paramMap.put("objectId", strTargetObjId);
                    paramMap.put("featureType",parentClonedObject.getTypeName());
                    paramMap.put("SelectionCriteria",ConfigurationConstants.RANGE_VALUE_MUST);
                    paramMap.put("languageStr",context.getSession().getLanguage());
                    paramMap.put("relId","");
                    paramMap.put("SourceRelId",strSourceRelId);
                    ...
                    //Code added for calling getXMLForSBCreateForMultipleFeatures
                    strSelectedObject = parentOID;
                    strObjectIdList.addElement(strSelectedObject);
                    relType = relType + "|" + strSourceRelId + "|" + strPartFamily;
                    mpSelectedFeatures.put(parentOID, relType);
                ...
                for(int j=iCount+1; j < selectedFeatureList.size(); j++) {
                    ...
                    if(!connectedOIDMap.containsKey(strNextObjLevelId) && strNextObjLevelId.startsWith(strFeatureLevelId+",") 
                         ...
                         childOID = childClonedObject.getId(context);
