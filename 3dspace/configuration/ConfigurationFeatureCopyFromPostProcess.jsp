     ...
     if(booelanDesignResp){
    	//part of process to clone multilevel structure
         ...
         if(reqMap.get("share").toString().equals("on")){      
             ...
             Map xmlMapAttributeVal = new HashMap();
             ...
             for(int iCount=0; iCount < selectedFeatureList.size(); iCount++){
				 ...
				 isAllowed = false;
				//code added for obtaining MapAttributeValues for xml call
					...
					MapList xmlMapAttrValues = new MapList();
  					Map xmlMapAttr = new HashMap();
  					xmlMapAttr.put("AttributekeyInType",featureMap.get("attribute["+ConfigurationConstants.ATTRIBUTE_KEY_IN_TYPE+"]"));
  					xmlMapAttr.put("AttributeAllowedFeatureSelections",featureMap.get("attribute["+ConfigurationConstants.ATTRIBUTE_CONFIGURATION_SELECTION_TYPE+"]"));
  					xmlMapAttrValues.add(xmlMapAttr);
  					xmlMapAttributeVal.put(featureToShare, xmlMapAttrValues);
				    if(typeShareObj){
				    	isAllowed = false;
             ...
             if(isAllowed){
            		if(flagValidationFailed == false){
            			if(!listSubFeats.contains(featureToShare)){
            				xml = ConfigurationFeature.getXMLForSBCreateForMultipleFeatures(context, paramMap, mpSelectedFeatures,xmlMapAttributeVal ,strObjectIdList);
            	           	xml = xml.replaceAll("pending", "committed");
            	           	%>
            	                <script language="javascript" type="text/javaScript">
            	                    ...
            	                    detailsDisplayFrame.editableTable.loadData();
            	                    detailsDisplayFrame.rebuildView();
            	                </script>
            	            <% 	
            			}
            		}
             ...
             <% 
                ...
                //taking xml API call outside for loop
                Map paramMap = new HashMap();
                Map mpSelectedFeatures = new LinkedHashMap();
                Map xmlMapAttributeVal = new HashMap();
                StringList strObjectIdList = new StringList();
                ...
                for(int iCount=0; iCount< selectedFeatureList.size(); iCount++){
                    ...
                    boolean typeCloneObj = new DomainObject(featureToClone).isKindOf(context,ConfigurationConstants.TYPE_CONFIGURATION_OPTION);
  					...
  					isAllowed = false;
  					//code added for obtaining MapAttributeValues for xml call
  					StringTokenizer strSelectedObjectTokenizer = new StringTokenizer(featureToClone,",");
  					for(int j = 0; j < strSelectedObjectTokenizer.countTokens(); j++){
  						strSelectedObject = strSelectedObjectTokenizer.nextElement().toString();
  						strObjectIdList.addElement(strSelectedObject);
  						break;
  					...
  					xmlMapAttr.put("AttributeAllowedFeatureSelections",featureMap.get("attribute["+ConfigurationConstants.ATTRIBUTE_CONFIGURATION_SELECTION_TYPE+"]"));
  					xmlMapAttrValues.add(xmlMapAttr);
  					xmlMapAttributeVal.put(featureToClone, xmlMapAttrValues);
		                    ...
		                    for(int j=iCount+1; j < selectedFeatureList.size(); j++) {
		                        ...
		                        if(!connectedOIDMap.containsKey(strNextObjLevelId) && strNextObjLevelId.startsWith(strFeatureLevelId+",") 
		                             ...
		                             if(vrvSubTypes.contains(childType)){
		                                 ...
		                                 relType = "type_ConfigurationOption";
		                             }
		                             mpSelectedFeatures.put(strNextObjId, relType);		                                                     
                ...
                //Code added for getXMLForSBCreateForMultipleFeatures call
                if(isAllowed){
                	if(connectedOIDMap.containsKey(strFeatureLevelId)){
                		 if(strParentKeyInType != null && !"".equals(strParentKeyInType) && !strParentKeyInType.equalsIgnoreCase("Blank") && !COTypes.contains(strFeatureType)){
                		}else if(vrvSubTypes.contains(strFeatureType) && !vrSubTypes.contains(strParentType) ){
                		...
                		}else{//selectedFeatureList try this as well instead of strObjectIdList
                			xml = ConfigurationFeature.getXMLForSBCreateForMultipleFeatures(context, paramMap, mpSelectedFeatures,xmlMapAttributeVal ,strObjectIdList);
                			xml = xml.replaceAll("pending", "committed");
                            %>
                                <script language="javascript" type="text/javaScript">
