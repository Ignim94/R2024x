  ...
  {	  
	  ...
	  if("ProductLineContext".equalsIgnoreCase(strMode))
	  {
          ...
          <script language="javascript" type="text/javaScript">     
		        var url= "../common/emxIndentedTable.jsp?toolbar=FTRVariabilityToolbar&expandProgram=ConfigurationFeature:expandVariabilityStucture&table=FTRProductLineVariabilityTable&editRootNode=false&selection=multiple&featureType=Marketing&header=emxConfiguration.Heading.Table.VariabilitySummary&sortColumnName=DisplayName&HelpMarker=emxhelpvariantoptionpage&massPromoteDemote=false&objectCompare=false&SuiteDirectory=configuration&suiteKey=Configuration&mode=<%=XSSUtil.encodeForJavaScript(context,mode)%>&editLink=<%=XSSUtil.encodeForJavaScript(context,editLink)%>&parentOID=<%=XSSUtil.encodeForJavaScript(context,strObjIdContext)%>&objectId=<%=XSSUtil.encodeForJavaScript(context,strObjIdContext)%>";
