...
%>
        	...
        	if(resp!=null){
        		if( resp.details != "undefined" && (("pid:"+rulePID) != resp.details.id)){
					...
					var defaultCategoryIndex = newURL.indexOf("DefaultCategory=");
					if(defaultCategoryIndex > 0){
						var defaultCategoryStr = newURL.substring(defaultCategoryIndex ,newURL.indexOf("&",defaultCategoryIndex) == -1 ?                            		newURL.length : newURL.indexOf("&",defaultCategoryIndex));
						newURL = newURL.replace(defaultCategoryStr,"");
					}
                 
					var parentOIDIndex = newURL.indexOf("parentOID=")+10;
					if(parentOIDIndex > 0){
						var parentOIDStr = newURL.substring(parentOIDIndex ,newURL.indexOf("&",parentOIDIndex) == -1 ?                                                  	newURL.length : newURL.indexOf("&",parentOIDIndex));
						newURL =  newURL.replace(new RegExp(parentOIDStr, 'g'), productPID);
					}
