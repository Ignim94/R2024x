...
<%
	...
	String invalidEffortsMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.WeeklyTimeSheet.Effort.validateNegativeandAlphabetEffortEntry", strLanguage);
	String effortsExceedLimitMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.WeeklyTimeSheet.Effort.validateEffortEntry", strLanguage);
	String negativeEffortsMessage = EnoviaResourceBundle.getProperty(context, "ProgramCentral","emxProgramCentral.WeeklyTimeSheet.Effort.validateNegativeEffort", strLanguage);
...
function checkParentChildDependancy(strEnteredDependancy,cellIdValue,cellRowId){
	...
	if(window && window.dataGridEnabled){
		...
		//IR-1267354
		// To avoid fetching value in DataGrid, when matchNode value is ""
		if(matchedNode !== ""){
...
function checkTransitiveDependancy(strEnteredDependancy,currentCellIdValue,editatedRowId){
	...
	var expectedRow = "";
		if(window && (window.name == "PMCWBSFlatView")){
			...
			//IR-1267354
			if(window && window.dataGridEnabled && expectedRowId==""){
				expectedRow = null;
			}else{
...
/*****************************************************************************************************/
/* function isValidEfforts() - checks whether the value of efforts is Valid or not*/
/*                                                                                                   */
/*****************************************************************************************************/

function isValidEfforts() {
    debugger;
    var efforts = arguments[0];
    // Check if efforts is empty or not a valid number
    if (efforts == "" || isNaN(efforts)) {
        alert("<%=invalidEffortsMessage%>");
        return false;
    }
    ...
    // Check if efforts is negative
    if (efforts < 0) {
        alert("<%=negativeEffortsMessage%>"); 
        return false;
    }

    // Check if efforts is more than 24 hours
    if (efforts > 24) {
        alert("<%=effortsExceedLimitMessage%>"); 
        return false;
    }
