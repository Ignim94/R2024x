	...
	function (STU, EP, Behavior, Event, DSMath, Actor3D) {
		...
		CarController.prototype.computeGravityForce = function (oMoveVec, iDeltaTime, iActor, iUpActorVec) {
			if (this.__internal__.HasWheels === true) {
				...
				var avgDstScene = 0;
				var avgNormalScene = new DSMath.Vector3D();
				var nbImpacts = 0;
				...
				for (var w in wheels) { /*jshint -W089 */
					...
					var wheelCenterPositionScene = DSMath.Transformation.multiply(actorTransformScene, this.__internal__.P13w[w]).vector; // dans le repere scene

					var rayVect = new STU.Ray();

					rayVect.origin.x = wheelCenterPositionScene.x - this.__internal__.Radius[w] * 4 * gravityDirectionScene.x; 
					rayVect.origin.y = wheelCenterPositionScene.y - this.__internal__.Radius[w] * 4 * gravityDirectionScene.y;
					rayVect.origin.z = wheelCenterPositionScene.z - this.__internal__.Radius[w] * 4 * gravityDirectionScene.z;

					wheelCenterAvgPosScene.addVector(wheelCenterPositionScene); // dans le repere scene

					rayVect.direction.x = gravityDirectionScene.x; // dans le repere scene
					rayVect.direction.y = gravityDirectionScene.y;
					rayVect.direction.z = gravityDirectionScene.z;
	                ...
	                var MyPos1 = DSMath.Transformation.multiply(actorTransformScene, this.__internal__.P13w[w]); // dans le repere scene
                    var Params1 = {
                        radius: 500,
                        screenSize: 0,
                        position: MyPos1,
                        referential:mySceneTransfo,
                        color: MyColor1,
                        alpha: 200,
                        lifetime: 1
                    };
                    ...
                    MyPos2.vector = intersectedPoint;
                    var Params2 = {
                        radius: 500,
                        screenSize: 0,
                        position: MyPos2,
				...
				wheelCenterAvgPosScene.multiplyScalar(0.25); // dans le repere scene
				if (nbImpacts > 0) {
					avgDstScene /= nbImpacts;
					if (nbImpacts === 4) {
						var rightVec = DSMath.Vector3D.sub(intersections.WheelBR, intersections.WheelBL).normalize(); // dans le repere scene
						...
						avgNormalScene = DSMath.Vector3D.cross(rightVec, frontVec); // dans le repere scene
					} else {
						avgNormalScene.multiplyScalar(1 / nbImpacts);
					}
					avgNormalScene.normalize(); // dans le repere scene
					...
					if (avgDstScene > ((10 + this.__internal__.Radius.WheelFR * 5))) {
						...
						var diffScene = (this.__internal__.Radius.WheelFR * 5) - avgDstScene; // dans repere scene
		...
		CarController.prototype.updateFollowPath = function (iDeltaTime) {
			...
			var mySceneTransfo = this.actor.getSceneTransform();
			var updatePosition = this.__internal__.target.getValue((this.__internal__.currentDistanceFollowPath) / this.__internal__.pathLength, mySceneTransfo);
		...
		CarController.prototype.updateWheelFollowPath = function (iDeltaTime) {
			...
			if (this.__internal__.HasWheels === true) {
				...
				var p1 = this.__internal__.target.getValue(distance_on_path_middle_front_wheel / this.__internal__.pathLength, mySceneTransfo);
				var p2 = this.__internal__.target.getValue(distance_on_path_end_front_wheel / this.__internal__.pathLength, mySceneTransfo);
		...
		CarController.prototype.init = function (iActor) {
			...
			var mySceneTransfo = this.actor.getSceneTransform();
			var upScene = mySceneTransfo.matrix.getThirdColumn().clone().normalize();
		...
		CarController.prototype.computeWheelRadius = function (iWheelBS, iWheel) {
			...
			if (oWheelRadius === -1) {
				...
				var CarScaleRelToScene = Math.cbrt(MyCar.getTransform("Location").matrix.determinant()); // don't use .getScaling().scale, it fails for transformations which are not perfectly orthogonal (planets)
