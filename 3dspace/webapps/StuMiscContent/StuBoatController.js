define('DS/StuMiscContent/StuBoatController',
	...
	function (STU, EP, Behavior, Task, EventServices, Event, DSMath, Actor3D) {
		...
		BoatController.prototype.onActivate = function (oExceptions) {
			...
			this._sceneTransform = this._actor.getSceneTransform();
		...
		BoatController.prototype.computeSize = function () {
			...
			//var SceneTransformation = this._actor.getSceneTransform();
			var ActorSceneTransformation = this._actor.getTransform("Location");
			var MyParams = { excludeChildren: 0, orientation: this._actor };

			var iBBox = this.actor.getOrientedBoundingBox(MyParams);
			var highBBox = iBBox.high.applyTransformation(ActorSceneTransformation);
			var lowBBox = iBBox.low.applyTransformation(ActorSceneTransformation);
		...
		BoatController.prototype.hitActor = function (iPosition, iDirection, iLength, iActor) {
			...
			var intersectArray = renderManager._pickFromRay(rayVect, false, false, this._Scene);
		...
		BoatController.prototype.projectTargetOnTheGround = function (iDestination) {
			...
			var pickingParams = {
				...
				reference: this._sceneTransform,
				pickGeometry: true,
				pickTerrain: true,
				pickWater: true
			};
		...
		BoatController.prototype.updateFollowTrajectory = function (iPath, iDeltaTime) {
			...
			if (iPath !== undefined || iPath !== null) {
				...
				var scale = this._sceneTransform.matrix.getScale();
		...
		BoatController.prototype.computeTrajectory = function () {
			...
			var s = this._sceneTransform.matrix.getScale();
			...
			v = this._sceneTransform.matrix.getThirdColumn();
			v.normalize();		

			for (var i = 0; i <= division; i++) {
				var pos = this._targetedPath.getValue(i / division, this._sceneTransform);
		...
		BoatController.prototype.drawSphere = function (iPosition, iSize, iIndexColor, iLifeTime) {
			...
			var iRef = this._actor.getSceneTransform();
			render._createSphere({ radius: iSize, position: localTransfo, color: iColor, alpha: 255, lifetime: lifeTime, reference:iRef });
		};
