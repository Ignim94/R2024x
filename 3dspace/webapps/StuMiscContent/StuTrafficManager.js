...
define('DS/StuMiscContent/StuTrafficManager', [
	...
	var TrafficPath = function (iPath) {
		...
		this.getSceneTransform = function () {
			return this.path.getSceneTransform();
		};
	};
    ...
    */
	...
	TrafficManager.prototype.computeGround = function (currentRoad) {
		...
		var mySceneTransfo = currentRoad.getSceneTransform();

		var currentPathPosition = currentRoad.getStartPosition(mySceneTransfo); //Scene
		...
		while (currentPathValue < currentRoad.getLength()) {
			...
			var newPosition = currentRoad.getValue(currentPathValue / currentRoad.getLength(), mySceneTransfo);
			newPosition.z = 0;
			...
			if (this.displayComputedPath) {
				var _scale = Math.cbrt(mySceneTransfo.matrix.determinant()); // don't use .getScaling().scale, it fails for transformations which are not perfectly orthogonal (planets)
				var tmpPoint = new DSMath.Point(trackingPosition.x, trackingPosition.y, trackingPosition.z);
				tmpPoint.applyTransformation(mySceneTransfo);
	...
	TrafficManager.prototype.computeTrafficWheelIntersections = function (iRoad, iTrackingPosition, iRightVector, iGroundIncluded) {
		...
		if (iGroundIncluded) {
			rightIntersection = this.getFirstIntersectPointDown(rightOrigin, mySceneTransfo);
			leftIntersection = this.getFirstIntersectPointDown(leftOrigin, mySceneTransfo);
		}
		else {
			var upVector = new DSMath.Vector3D(0, 0, 1);
	...
	TrafficManager.prototype.getAverageIntersection = function (intersectionList, value) {
		...
		//We suppose that all path are under the same scene... (not necessarily true)
		var mySceneTransfo = this._trafficPaths[0].getSceneTransform();
		...
		if (percent === 0) {
			if (intersect1.normal !== undefined && intersect1.normal !== null && intersect2.normal !== undefined && intersect2.normal === null) {
				...
				rightVec.applyTransformation(mySceneTransfo);
     ...
     * @private
     */
	TrafficManager.prototype.getFirstIntersectPointDown = function (position, iSceneTransfo) {
        ...
        var intersectArray = renderManager._pickFromRay(rayVect, true, true, iSceneTransfo);
