...
//     	  	});
		...
		/// decides whether or not to trigger quality check
			...
			if (that.isOpen) {
				...
				if (that.checkSelection()){
					...
					const selectedNode = PADContext.get().getSelectedNode();
				
					if (UWA.is(selectedNode) && (that.isLoading || !UWA.is(that.lastAssessedReq) || that.lastAssessedReq !== selectedNode.options.elementID || that.selReqHasBeenChanged)) {
		...
		/// gets the selected requirement and triggers the quality check
			...
			var node = jQuery("#" + selectedNode.options.elementID + "_content");
			...
			this.lastAssessedReq = selectedNode.options.elementID;
...
//			};
		...
		/// returns the currently in the RichView selected element
			...
			if (selectionExists) {
				return PADContext.get().getSelectedNode();
			} else {
				return undefined;
			}
		},
		...
		/// checks if current selection is a requirement or not
			...
			if (selectedElement !== undefined && PADContext.get().getSelectedNode().options.kindof === 'Requirement') {
				return true;
			} else {
		...
		/// returns the type of the currently selected element in the RichView
			...
			if (selectedElement !== undefined && UWA.is(PADContext.get().getSelectedNode().options.kindof)) {
				return PADContext.get().getSelectedNode().options.kindof;
			} else {
				return '';
			}
		},
...
//		},
		...
		/// highlights metric errors in richview elements
			...
			var selectedNode = PADContext.get().getSelectedNode();
			var objectID = selectedNode.options.elementID;
