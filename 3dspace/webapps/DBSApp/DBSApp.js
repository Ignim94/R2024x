					...
					myFields.push({
						...
						html: new function() {
							...
							var toggle = new Toggle({
								...
								disabled: !dicoHandler.isAuthoring, // FUN147722:  || model.get('automatic')==='No',
								label: myNls.get("interfaceFormAutomaticFieldOption"),
...
define('DS/DBSApp/Utils/DMSWebServices', [
	...
	var DMSWebServices = Class.singleton({
		...
		launchExport: function(onSuccess, onError, onMajorExport, debugExport) {
			var self = this;
			function processExport(data, onResult) {
				if(!data) {
					onError()
				}
				if(onResult) return dmsRequest({
					method: 'POST',
					...
					onComplete: function(response, headers, request) {
						var payload = self.generateDataForImpacts(response);
						if(!payload.isMajor && !payload.isPropagation) { 
							processExport(data);
						} else {
							onResult(data, payload, processExport);
						}
					},
					data: JSON.stringify(data),
					headers: {
				...
				if(debugExport) return dmsRequest({
					method: 'POST',
					dicoapi: "/resources/dictionary/ExportData",
					restargs: ["lang=" + widget.lang, "filename=DMSSignOff"],
					onError: onError,
			...
			processExport({ Dictionary: {} }, onMajorExport);

		},
		generateDataForImpacts (response) {
			var report = Object.entries(response.impacts).map(([adminObject, impact])=>({
				"nature": adminObject.split('.')[0],
				"fullname": adminObject.split('.').splice(1).join('.'),
				"impact": impact,
				"isMajor": impact.indexOf("INDEX_UPGRADE")>=0 || impact.indexOf("DATABASE_UPGRADE")>=0 || impact.indexOf("ATTRGROUP_UPGRADE")>=0,
				"isPropagation": impact.indexOf("ATTRGROUP_UPGRADABLE")>=0,
				"targetCost": parseInt("0" + (response.targets || {})[adminObject]),
				"estimatedCost": parseInt("0" + (response.estimation || {})[adminObject])
			}));
			var impacts = report.slice().filter(item => item.nature=='Type' || item.nature=='Relationship')
							.map(item => Object.assign(item, {
								"multiplier": item.isMajor ? Math.max(1, report.filter(inner=>inner.nature=="Attribute" && inner.fullname.startsWith(item.fullname + '.') && inner.isMajor).length) : 0,
								"attributes": item.isMajor ? report.filter(inner=>inner.nature=="Attribute" && inner.fullname.startsWith(item.fullname + '.') && inner.isMajor) : [],
								"unqualifiedAttributes": report.filter(inner=>inner.nature=="Attribute" && inner.fullname.startsWith(item.fullname + '.') && !inner.isMajor)
							}))
							.filter(item=>item.isMajor);
			var unqualifiedImpacts = report.filter(item => item.nature=='Type' || item.nature=='Relationship')
										.filter(item=> !item.isMajor && item.isPropagation);

			var propagation = report.filter(item => item.nature=='Interface' && item.isPropagation)
								.map(item => Object.assign(item, {
									"multiplier": Math.max(1, report.filter(inner=>inner.nature=="Attribute" && inner.fullname.startsWith(item.fullname + '.')).length),
									"attributes": report.filter(inner=>inner.nature=="Attribute" && inner.fullname.startsWith(item.fullname + '.'))
								}));
			
			var payload = {
				impact: Object.fromEntries(impacts.map(item=>[item.nature + '.' + item.fullname, item])),
				unqualifiedImpacts: Object.fromEntries(unqualifiedImpacts.map(item=>[item.nature + '.' + item.fullname, item])),
				...
				isPropagation: propagation.some(item=>item.isPropagation),
...
function (UWA, Alert, Button, Toggle, Modal, Popover, SuperModal, Tooltip, dicoHandler, myNls) {
	'use strict';
	...
	function checkAllInputsValid(data, exportCountValues) {
		...
		let isValid = true;
		for(let key in data.impact) { 
			if(!exportCountValues[key] || !exportCountValues[key]["PopulationSignOff"] || exportCountValues[key]["PopulationSignOff"] == '') {
				isValid = false;
			}
		}
		return isValid;
	...
	function checkPopulationCount(data, exportCountValues, limit) {
		...
		let sum = 0;
		for(let key in data.impact) {
			sum += data.impact[key]["multiplier"] * exportCountValues[key]["PopulationSignOff"];
		}
		return sum <= limit;
	}
    ...
    function AttrPopulationCountModal(container, opts) {
		...
		this.displayOption = opts.displayOption;
    ...
    AttrPopulationCountModal.prototype = {
		...
		//Modal for the export fun : FUN Add Attribute 
		formModal: function(options) {
			var self = this;
			//tag div element
			var headerDiv = UWA.createElement('div', {
		...
		getTableDataCheckboxElement: function(opts) {
			...
			checkbox.elements.container.style.paddingLeft = '28px';
			if(opts.isChecked) checkbox.check();
			return checkbox;
		},
		getValueOfTableInput: function(impact) {
			var key = impact.nature + "." +  impact.fullname;
			if(document.getElementById(key + '_input')) {
				let populationSignOffTmp = document.getElementById(key + '_input').value  ? parseInt('0' + document.getElementById(key + '_input').value) : ''
				this.exportCountValues[key] =  {
					PopulationSignOff: populationSignOffTmp
				};
				
				this.changeRowInputColor(key);
				this.changeExportModalOpsCount(key, impact);
				// TODO: UniqueKeys to be defined
				let keyType = key.replace(/^(Type|Relationship)\./, "");
				if(impact.nature == "Type") {
					this.requestItf.Dictionary.Types[keyType] =  {
						PopulationSignOff: parseInt('0' + document.getElementById(key + '_input').value)
					};
				} else if(impact.nature == "Relationship") {
					this.requestItf.Dictionary.Relationships[keyType] =  {
						PopulationSignOff: parseInt('0' + document.getElementById(key + '_input').value)
					};
				}
		...
		changeExportModalOpsCount: function(key, impact) {
			let element = document.getElementById(key + '_input');
			if(element) {
				if(element.value && element.value!= '') {
					document.getElementById(key + '_count').textContent = element.value * impact["multiplier"] + myNls.AttrPopulationCountModalExportOperations;
					if(impact["attributes"].length) {
						for(let attribute of impact["attributes"]) {
							document.getElementById(attribute.fullname + '_count').textContent = element.value + myNls.AttrPopulationCountModalExportOperations;
						}
					}
				} else {
					document.getElementById(key + '_count').textContent = '';
					if(impact["attributes"].length) {
						for(let attribute of impact["attributes"]) {
							document.getElementById(attribute.fullname + '_count').textContent = '';
						}
					}
				}
			}
		...
		reRenderExportModal: function(myContent, myModal) { 
			...
			if(isAllInputsValid) var isPopulationCountValid = checkPopulationCount(this.data, this.exportCountValues, this.maxUpgradeOpCount); 
		...
		getModalAlert: function(opts) {
			...
			this.getModalAlertMessage(this.modalContent, {
		...
		renderExportFooter : function(myContent, myModal, isValidate) {
			...
			cancelButton.addEvent('onClick', function () {
				this.process();
				myModal.next(); // next method for SuperModal
			}.bind(this));

			if(isValidate) {
				var validateButton = new Button({ value: myNls.ValidateExportBtn, id:'validateBtn', className: 'primary' });

				validateButton.addEvent('onClick', function () {
					this.process(this.requestItf);
					myModal.next(); // next method for SuperModal
				}.bind(this));
		...
		renderImportFooter : function(myContent, myModal, isValid) {
			...
			if(isValid) {
				...
				validImportButton.addEvent('onClick', function () {
					myContent.dispatchEvent(new CustomEvent('validateImportEvent', {
		...
		getModalAlertMessage : function(modalContent, options) {
		...
		collapseLine: function(contentValue, lineData, event) {
			document.getElementById(contentValue + '_collapse_btn').classList.toggle("active-collapse");
			if(lineData["attributes"].length) {
				for(let attribute of lineData["attributes"]) {
					document.getElementById(attribute.fullname + '_row').classList.toggle("custom-collapse_tr");
				}
				if(this.displayVersion.toLowerCase() == "import") {
					let isActive = document.getElementById(contentValue + '_collapse_btn').classList.contains("active-collapse");
					let rowSpanLen = isActive ? (lineData["attributes"].length+1) : 1;
					document.getElementById(contentValue + '_targetSize').rowSpan = rowSpanLen;
					document.getElementById(contentValue + '_targetSize').classList.toggle("custom-td-center");
					document.getElementById(contentValue + '_compliance').rowSpan = rowSpanLen;
					document.getElementById(contentValue + '_compliance').classList.toggle("custom-td-center");
				}
			}
			if(lineData["unqualifiedAttributes"].length) {
				for(let attribute of lineData["unqualifiedAttributes"]) {
					document.getElementById(attribute.fullname + '_row').classList.toggle("custom-collapse_tr");
				}
			}
		},
		getExportInterfacesValues : function(modal, nextStepFn) { 
			let interfacesChecked = {};
			for(let contentValue of this.data.propagation) {
				let element = document.getElementById(contentValue.fullname + '_checkbox')
				if(element.checked) {
					interfacesChecked[contentValue.fullname] = {
						"Name" : contentValue.fullname,
						"Nature" : contentValue.nature,
						"AutomaticUpgrade": "Yes"
					};
				}
			}
			if(interfacesChecked) this.requestItf.Dictionary.Interfaces = interfacesChecked;
			this.process(this.requestItf, nextStepFn);

			// make the waiting status
			document.body.style.cursor = "wait"; 
			if(document.getElementById("custom-nextStep-btn")) document.getElementById("custom-nextStep-btn").disabled = true;
		},
		getExportFirstModal : function (theadValues, data) { 
			...
			let count = 1;
			for(let contentValue of data.propagation) {
				var trElement = UWA.createElement('tr', {
					'id': contentValue.fullname + '_row'
				}).inject(tbody);

				this.getTableDataCheckboxElement({ 
					label: dicoHandler.getDisplayName(contentValue.fullname, contentValue.nature),
					id: contentValue.fullname + '_checkbox',
					disabled: contentValue.attributes.length ? false : true,
					isChecked: this.requestItf.Dictionary.Interfaces && this.requestItf.Dictionary.Interfaces[contentValue.fullname]
				}).inject(trElement);

				let typeListStr = "";
				let typeArr = dicoHandler.attributeGroupHadScope(contentValue.fullname);
				if(typeArr.length > 0) {
					typeListStr = dicoHandler.getDisplayName(typeArr[0], "Type");
					for(let i = 1; i<typeArr.length; i++) {
						typeListStr += (", " + dicoHandler.getDisplayName(typeArr[i], "Type"));
					}
				}
				var typeListSubStr = typeListStr;
				if(typeListStr.length > 27) { // first 27 chars
					typeListSubStr = typeListStr.substring(0, 27) + " ...";
				}

				var typeElement = this.getTableDataElement({
					headerName: typeListSubStr,
					width: "450px",
					headerId: "nameColumnValue",
					id: contentValue.fullname + '_type'
				}).inject(trElement);

				if(typeListStr.length > 27) {
					new Tooltip({
						target: typeElement,
						body: typeListStr
					});
				...
				this.getTableDataElement({
					headerName: contentValue.attributes.length + myNls.AttrPopulationCountModalExportAttribute,
					width: "450px",
					headerId: "nameColumnValue",
					id: contentValue.fullname + '_attribute'
				}).inject(trElement);

				if(count % 2 != 0) {
					trElement.style.backgroundColor = "#F9F9F9";
				}
				count ++;
			}

			var myModalContent = UWA.createElement('div', {
				'id': "myModalContent",
				"style": {
					"justify-content": "center"
				}
			});
			this.modalContent = myModalContent;
			table.inject(myModalContent);
			
			return myModalContent;
		},
		getExportSecondModal : function (theadValues, data) { 
			var table = UWA.createElement('table', {
				'class': 'table table-bordered',
				'id': 'attrTable',
				'styles': {
			...
			for(let impactedItem in data.impact) {
				var trElement = UWA.createElement('tr', {
					'id': data.impact[impactedItem].fullname + '_row'
				}).inject(tbody);

				var typeElement = this.getTableDataElement({
					headerName: dicoHandler.getDisplayName(data.impact[impactedItem].fullname, data.impact[impactedItem].nature),
					width: "450px",
					headerId: "nameColumnValue",
					id: impactedItem + "_name"
				}).inject(trElement);

				var collapseBtn = UWA.createElement('i', {
					id: impactedItem + '_collapse_btn',
					class: 'custom-collapse-btn'
				}).inject(typeElement);

				collapseBtn.onclick = this.collapseLine.bind(this, impactedItem, data.impact[impactedItem]);

				var input = UWA.createElement('input', {
					'class': 'input-error',
					'type': 'number',
					'id': impactedItem + '_input',
					'min': 0,
				...
				input.onblur = this.getValueOfTableInput.bind(this, data.impact[impactedItem]);
				...
				this.getTableDataElement({
					...
					width: "450px",
					headerId: "nameColumnValue",
					id: impactedItem + "_count"
				}).inject(trElement);

				// render sub attributes rows
				if(data.impact[impactedItem]["attributes"].length) {
					for(let attribute of data.impact[impactedItem]["attributes"]) {
						var trElement = UWA.createElement('tr', {
							id: attribute.fullname + '_row', 
							class: 'custom-collapse_tr'
						}).inject(tbody);
						let attributeNamesArr = attribute.fullname.split(".");
						let attributeDisplayedName = dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-1], attribute.nature);
						if(attributeNamesArr.length > 2) {
							attributeDisplayedName += " (" + dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-2], data.impact[impactedItem].nature) + ")";
						}
						this.getTableDataElement({
							headerName: '\xa0\xa0\xa0\xa0\xa0\xa0' + attributeDisplayedName,
							width: "450px",
							headerId: "nameColumnValue",
							id: attribute.fullname + "_name",
							colspan: 2
						}).inject(trElement);
						...
						this.getTableDataElement({
							...
							width: "450px",
							headerId: "nameColumnValue",
							id: attribute.fullname + "_count"
						}).inject(trElement);
					}
				}

				//render sub unqualifiedAttributes rows 
				if(data.impact[impactedItem]["unqualifiedAttributes"].length) {
					for(let attribute of data.impact[impactedItem]["unqualifiedAttributes"]) {
						var trElement = UWA.createElement('tr', {
							id: attribute.fullname + '_row', 
							class: 'custom-collapse_tr'
						}).inject(tbody);
						...
						this.getTableDataElement({
							headerName: myNls.AttrPopulationCountModalExportNotPropagated,
							width: "450px",
							headerId: "nameColumnValue",
							id: attribute.fullname + "_input"
						}).inject(trElement);
		
						this.getTableDataElement({
							headerName: 0 + myNls.AttrPopulationCountModalExportOperations,
							width: "450px",
							headerId: "nameColumnValue",
							id: attribute.fullname + "_count"
						}).inject(trElement);
			...
			// render all the types not chosen by the user in the first panel of Export 
			for(let impactedItem in data.unqualifiedImpacts) {
				var trElement = UWA.createElement('tr', {
					'id': data.unqualifiedImpacts[impactedItem].fullname + '_row'
				}).inject(tbody);

				var typeElement = this.getTableDataElement({
					headerName: dicoHandler.getDisplayName(data.unqualifiedImpacts[impactedItem].fullname, data.unqualifiedImpacts[impactedItem].nature),
					width: "450px",
					headerId: "nameColumnValue",
					id: impactedItem + "_name"
				}).inject(trElement);
				...
				collapseBtn.onclick = this.collapseLine.bind(this, impactedItem, data.unqualifiedImpacts[impactedItem]);
				...
				// render sub unqualifiedAttributes rows 
				if(data.unqualifiedImpacts[impactedItem]["unqualifiedAttributes"].length) {
					for(let attribute of data.unqualifiedImpacts[impactedItem]["unqualifiedAttributes"]) {
						var trElement = UWA.createElement('tr', {
							id: attribute.fullname + '_row', 
							class: 'custom-collapse_tr'
						}).inject(tbody);
						...
						let attributeDisplayedName = dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-1], attribute.nature);
						if(attributeNamesArr.length > 2) {
							attributeDisplayedName += " (" + dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-2], data.unqualifiedImpacts[impactedItem].nature) + ")";
						}
						this.getTableDataElement({
							headerName: '\xa0\xa0\xa0\xa0\xa0\xa0' + attributeDisplayedName,
							width: "450px",
		...
		getImportModalPanel: function (myContent, theadValues, impacts) { 
			// FUN : Propagate Attribute
			var self = this;
			...
			for(let key in impacts) {
				var trElement = UWA.createElement('tr').inject(tbody);
				let methodName = this.displayOption || widget.getValue("DisplayOption")==='NLSOption' ? 'getNLSName' : 'getDisplayName';
				var typeElement = this.getTableDataElement({ 
					headerName: dicoHandler[methodName].call(dicoHandler, impacts[key].fullname, impacts[key].nature),
					width: "250px",
					headerId: "nameColumnValue",
					id: impacts[key].nature + "." + impacts[key].fullname + "_name"
				}).inject(trElement);
				
				var collapseBtn = UWA.createElement('i', {
					id: impacts[key].nature + "." + impacts[key].fullname + '_collapse_btn',
					class: 'custom-collapse-btn'
				}).inject(typeElement);

				collapseBtn.onclick = this.collapseLine.bind(this, impacts[key].nature + "." + impacts[key].fullname, impacts[key]);

				this.getTableDataElement({
					headerName: impacts[key].targetCost,
					width: "250px",
					headerId: "nameColumnValue",
					id: impacts[key].nature + "." + impacts[key].fullname + "_targetSize"
				}).inject(trElement);

				this.getTableDataElement({
					headerName: myNls["AttrPopulationCountModalImportDatabaseCheck" + (impacts[key].targetCost<impacts[key].estimatedCost ? "KO" : "OK")].replaceAll("%SIZE%", impacts[key].targetCost),
					width: "250px",
					headerId: "nameColumnValue",
					id: impacts[key].nature + "." + impacts[key].fullname + "_compliance"
				}).inject(trElement);

				// upgradeCount
				// this.getTableDataElement({
				// 	headerName: impacts[key].targetCost * impacts[key]["attributes"].length,
				// 	width: "250px",
				// 	headerId: "nameColumnValue"
				// }).inject(trElement);

				// render sub attributes rows
				if(impacts[key]["attributes"].length) {
					for(let attribute of impacts[key]["attributes"]) {
						var trElement = UWA.createElement('tr', {
							id: attribute.fullname + '_row', 
							class: 'custom-collapse_tr'
						}).inject(tbody);
						...
						let attributeDisplayedName = dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-1], attribute.nature);
						if(attributeNamesArr.length > 2) {
							attributeDisplayedName += " (" + dicoHandler.getDisplayName(attributeNamesArr[attributeNamesArr.length-2], impacts[key].nature) + ")";
						}
						this.getTableDataElement({
							headerName: '\xa0\xa0\xa0\xa0\xa0\xa0' + attributeDisplayedName,
							width: "250px",
						...
						// this.getTableDataElement({
						// 	headerName: impacts[key].estimatedCost / impacts[key]["attributes"].length,
						// 	width: "250px",
						// 	headerId: "nameColumnValue",
						// 	id: attribute.fullname + "_count"
						// }).inject(trElement);
						...
						// 	width: "250px",
						// 	headerId: "nameColumnValue",
						// 	id: attribute.fullname + "_upgradeCount"
						// }).inject(trElement);
					}
				}
		...
		getExportTwoStepModal: function(myModal) {
			var self = this;
			myModal.dialog({
				body : this.getExportFirstModal(this.theadValuesExt, this.data), 
				title : this.heading,
				buttons: [{
					className: 'primary',
					value: myNls.NextStepBtn,
					id: "custom-nextStep-btn",
					action: (modal) => self.getExportInterfacesValues(modal,  function(data, response, processExport) {
						document.body.style.cursor = ""; 
						this.data = response;
						myModal.dialog({
							body : this.getExportSecondModal(this.theadValues, this.data), 
							title : this.heading,
							buttons: [{
								className: 'default',
								id: 'cancelBtn',
								value: myNls.CancelExportBtn,
								action: function (modal) {
									self.process();
									modal.hide();
								}
							},{
								className: 'default custom-back-btn',
								id: 'returnBtn',
								value: myNls.ReturnBtn,
								action: function (modal) {
									modal.hide();
									self.getExportTwoStepModal(myModal);
								}
							}]
						});
						
						this.modal.next();
						this.isExportFinal = true; 
						this.getModalAlert({
							className: 'primary',
							message: myNls.AttrPopulationCountModalExportMissingInput
						});
			...
			this.getModalAlert({
				className: 'primary',
				message: myNls.AttrPopulationCountModalExportSelectAttrGrp
			});
		},
		build: function() {
			var self = this;
			...
			this.requestItf = { Dictionary: {Types: {}, Relationships: {}}};
			this.isExportFinal = false;
			if(this.displayVersion.toLowerCase() == "export") {
				var myContent = this.myContent;
				var myModal = this.modal = new SuperModal({
					className: 'fancy',
					visible: true,
					renderTo: this.container,
					closable: false,
					events: {
						onHide: function() {
							if(this.isExportFinal) {
								myModal.destroy();
							}
						},
						onShow: function() {
							document.getElementsByClassName('modal-wrap')[0].setStyle("width", '800px');
						}
					}
				});
				...
				// Step 1 : Export Panel 
				this.theadValuesExt = [{
					title: myNls.AttrPopulationCountModalExportAttrGroup,
					width: '33%'
				},{
					title: myNls.AttrPopulationCountModalExportScope,
					width: '33%'
				},{
					title: myNls.AttrPopulationCountModalExportImpact,
					width: '33%'
				}];

				if(this.data.propagation.length) {
					this.getExportTwoStepModal(myModal);
				} else {
					// skip the first modal
					myModal.dialog({
						body : this.getExportSecondModal(this.theadValues, this.data), 
						title : this.heading,
						buttons: [{
				...
				this.getImportModalPanel(this.myContent, this.theadValues, Object.values(this.data.impact).filter(item=>item.isMajor));
				if(!this.isValid) {
					this.modalContent = this.myContent;
					this.getModalAlert({
						className: 'error',
...
function (UWA, View, Scroller, Select, Popover, Button, Alert, myNls, Mask, dicoHandler, DMSWebServices, AttrPopulationCountModal, Spinner/*,PlatformAPI*/) {
	'use strict';
	...
	var extendedView = View.extend({
		...
		*/
		...
		buildMenu: function () {
			...
			this.divImportSchemaHistory = UWA.createElement('div', {
				'styles': {
    				'justify-self': 'left',
					'margin-bottom': '10px'
				}
			}).inject(divGridContent);
			this.divImportSchemaHistory.style.gridColumn = '3'
			this.divImportSchemaHistory.style.gridRow = '3'
			...
			this.divUpdateIndexHistory.style.gridColumn = '3'
			this.divUpdateIndexHistory.style.gridRow = '4'
				...
				events: {
					onClick: function (event) {
						...
						DMSWebServices.launchUpdateIndex(
							this.onCompleteRequestUpdateIndex.bind(this), 
							this.onFailureRequestUpdateIndex.bind(this),
							event && event.ctrlKey
						);
					}.bind(this)
				}
		...
		onCompleteRequestImport: function (payload) {
			...
			setTimeout(this.launchGetUpdateIndexInfo.bind(this), "5000");
		},
		...
		onMajorRequestImport: function(response, processor) {
			...
			new AttrPopulationCountModal(this.container, { //TODO: mettre les détails de rendering dans AttrPopulationCountModal?
				...
				theadValues: [
					{
						"title" : myNls.AttrPopulationCountModalImportTypes
					},
					{
					...
					// {
					// 	"title" : myNls.AttrPopulationCountModalExportOperationsCalcul
					// }
				],
				data: response,
				isValid: /*response.isValid &&*/ response.estimatedCost <= dicoHandler.maxUpgradeOpCount, // We no longer care about individual estimation
		...
		onMajorRequestExport: function (request, response, processExport) {
			...
			new AttrPopulationCountModal(this.container, {
				myContent: myContent,
				theadValues :  [{
					"title" : myNls.AttrPopulationCountModalExportTypes 
				},
				{
				...
				process: function(data, onResult) {
					if(!data) { // Cancel export
						Mask.unmask(this.contentDiv);
					} else { // Attempt export again
						processExport(data, onResult);
					}
				}.bind(this),
				maxUpgradeOpCount: dicoHandler.maxUpgradeOpCount,
		...
		launchGetUpdateIndexInfo: function () {
			DMSWebServices.isDMSAccessible(
				function(payload) {
					if(payload['ImportSchema']['status'] === "OnGoing" || payload['UpdateIndexModel']['status'] === "OnGoing" || payload['CloudviewStatus']['status'] === "OnGoing") {
						this.updateButton.setDisabled(true);
						this.importButton.setDisabled(true);
					} else {
						this.updateButton.setDisabled(false);
						this.importButton.setDisabled(document.getElementById('ImportFileInput').files.length!==1);
					}
					this.updateJob(this.divImportSchemaHistory, payload['ImportSchema']);
					if(payload['ImportSchema']['status']==='OnGoing') {
						this.updateJob(this.divUpdateIndexHistory, {}); // Do not display both status if import is running
					} else if(payload['CloudviewStatus'] && payload['CloudviewStatus']['status']=='OnGoing' && payload['UpdateIndexModel']['status']!=='OnGoing') {
						this.updateJob(this.divUpdateIndexHistory, Object.assign({}, payload['CloudviewStatus'], { // Tenant isolation is indexing is running somewhere else
							'User': '', 'Reason': '',
							'LastSucceeded': payload['UpdateIndexModel']['LastSucceeded']
						}));
					} else {
						this.updateJob(this.divUpdateIndexHistory, payload['UpdateIndexModel']); // Display this tenant status 
					}
				}.bind(this),
				function (payload) {
					UWA.log("Failure to retrieve update information: " + payload);
					Mask.unmask(this.contentDiv);
					this.userMessaging.add({ className: "error", message: payload});
				}.bind(this)
			);
			return;
		...
		updateJob: function(jobElt, payload) {
			jobElt.empty();
			if(!!payload['status']) jobElt.addContent({
				'tag': 'div',
				'styles': {
					'line-height': '1.0em',
				...
				'html': [
					payload['status']=='OnGoing' ? new Spinner({ 
						'className': "small", 'visible': true
					}) : {
						'tag': 'span',
						'class': payload['status']=='Ended' ? 'fonticon  fonticon-check' : 'fonticon  fonticon-attention',
						'styles' : {
							'color':  payload['status']=='Ended' ? 'green' : 'red',
							'padding': '4px'
						}
					}, {
						'tag': 'span',
						'text': 
							payload['status']=='OnGoing' ? myNls.UpdateInProgress  :
							payload['status']=='Failed'  ? myNls.updateIndexfail   :
							!!payload["LastSucceeded"]   ? myNls.lastSuccessUpdate :
							payload['status']=='Ended'   ? myNls.updateIndexDone   : '',
						'styles': {
							'font-size': '1.2em'
						}
					}, {
						'tag': 'span',
						'text': payload['status']=='Ended' && !!payload["LastSucceeded"] ? new Date (parseInt('0' + payload["LastSucceeded"], 10)).toLocaleString() : '',
						'styles': {
							'white-space': 'nowrap'
						}
					}
			...
			if(!!payload['status'] && !!payload["startedAt"]) jobElt.addContent({
				'tag': 'div',
				'styles': {
					'display': 'inline-block',
					'font-size': '0.8em',
				...
				'html': [
					{
						'tag': 'span',
						'class': 'fonticon  fonticon-play', 
						'styles': {
						}
					},
					{
						'tag': 'span',
						'text': new Date (parseInt('0' + payload["startedAt"],10)).toLocaleString(),
						'styles': {
						}
					}
				]
			});
			if(!!payload['status'] && !!payload["EndedAt"] && payload["status"]!=="OnGoing") jobElt.addContent({
				'tag': 'div',
				'styles': {
					'display': 'inline-block',
					'font-size': '0.8em',
				...
				'html': [
					{
						'tag': 'span',
						'class': 'fonticon  fonticon-to-end',
						'styles': {
						}
					}, {
						'tag': 'span',
						'text': new Date (parseInt('0' + payload["EndedAt"],10)).toLocaleString(),
						'styles': {
						}
					}
				]
			});
			if(!!payload["status"] && !!payload["User"]) jobElt.addContent({
				'tag': 'div',
				'styles': {
					'font-size': '0.8em',
					'margin-left': '10px',
				...
				'html': [
					{
						'tag': 'span',
						'class': 'fonticon fonticon-user', 
						'styles': {
						}
					},
					{
						'tag': 'span',
						'text': payload["User"] + "  " + (myNls[payload["Reason"]] || payload["Reason"]),
						'styles': {
						}
					}
				]
			});
			if (!!payload['status'] && !!payload["LastSucceeded"] && payload['status']!='Ended' ) jobElt.addContent({
				'tag': 'div',
				'styles': {
					'font-size': '0.8em',
					'margin-left': '10px'
				...
				'html': [
					...
					{
						'tag': 'span',
						'text': myNls.lastSuccessUpdate + " : " + new Date (parseInt('0' + payload["LastSucceeded"],10)).toLocaleString()
					}
				]
			});
		},
		...
		setUpdateIndexInformation: function (status, startDate, endDate, lastsuccesfulDate, user, reason) {
			...
			pUserReason.setText(user + " - " + (myNls[reason] || reason));
