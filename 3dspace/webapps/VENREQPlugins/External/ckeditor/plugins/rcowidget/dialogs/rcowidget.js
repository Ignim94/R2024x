 ...
 *
 * Author: TRM Team
 * @quickreview UW9 VAI1 2024:07:08  IR-1263738 / IR-1274061 : Tenant will get appended in every URL request
 * @quickreview ZUD SSE17 2024:06:21  IR-1281449 - Saving of CEStamp is done from function setCEStamp.
 * @quickreview SSE17 KIE1 2023:06:25 IR-959040 : Empty/incomprehensible error message in Requirement widget when network is lost
 * @quickreview UW9 ZUD  22:11:17  IR-995996: cestamp needed to be updated after rco creation
 */
    ...
    .done(function(script, textStatus) {
        ...
        dropzone.on('success', function (file, serverResponse) {
            ...
            if(UWA && UWA.is(window.widget)) {
                ...
                require(['DS/WAFData/WAFData', 'DS/PADUtils/PADContext', 'DS/TraceableRequirementsUtils/Services/RequirementWebServices',
                         'DS/TraceableRequirementsUtils/Utils',
                         'DS/TraceableRequirementsUtils/utils/RequirementAlertManager',
                         'DS/TraceableRequirementsUtils/utils/RequirementErrorManager'
                         ], function(WAFData, PADContext, RequirementWebServices, REQUtils, RequirementAlertManager,RequirementErrorManager) {
                    ...
                    fetchOpts.onComplete = function(result) {
                        //IR-995996 - call to getInfo to update cestamp
                         RequirementWebServices.getInfo({
                                securityContext: REQUtils.getSecurityContext(),
                                pid: physicalId,
                                authored: true,
                                selectables: 'cestamp',
                                onComplete: function (getInfoResponse) {
                                    getInfoResponse = UWA.is(getInfoResponse, 'string') ? JSON.parse(getInfoResponse) : getInfoResponse;
                                    var results = getInfoResponse.results[0];
                                    let NodesToUpdateCeStamp=context.options.model.getNodesById(physicalId);
                                    //update CEStamp for all instances of node
                                    for(let indx =0; indx<NodesToUpdateCeStamp.length; indx++)
                                        NodesToUpdateCeStamp[indx].setCEStamp(results.cestamp);                                    
                                },
                                onFailure: function (err) {
                                    var message = RequirementErrorManager.getParseErrorMessage(err);   
                                    RequirementAlertManager.errorAlert(message);
                                },
                    ...
                    url = REQUtils.appendTenantInUrl(url);
                                                ...
                                                if(UWA && UWA.is(window.widget)) {
                                                    ...
                                                    if(isCreationAuthorized !== false) {
                                                        ...
                                                        require(['DS/TraceableRequirementsUtils/Services/RichContentObjectServices',
                                                         ...
                                                         ], function(RCOServices, RequirementWebServices, REQUtils, RequirementAlertManager, RequirementErrorManager,PADContext) {
                                                            ...
                                                            _rcoService.checkinFiles({
                                                                ...
                                                                callbacks: {
                                                                    onComplete: function(response, options) {
                                                                         RequirementWebServices.getInfo({
                                                                                ...
                                                                                onComplete: function (getInfoResponse) {
                                                                                    getInfoResponse = UWA.is(getInfoResponse, 'string') ? JSON.parse(getInfoResponse) : getInfoResponse;
                                                                                    var results = getInfoResponse.results[0]; 
                                                                                    let NodesToUpdateCeStamp=widget_context.options.model.getNodesById(physicalid);
                                                                                    //update CEStamp for all instances of node
                                                                                    for(let indx =0; indx<NodesToUpdateCeStamp.length; indx++)
                                                                                        NodesToUpdateCeStamp[indx].setCEStamp(results.cestamp);                                                                                                                                                     
                                                                                    if(widget_context) {
                                                                                        RequirementAlertManager.sucessAlert('File uploaded.');
                                                                    ...
                                                                    onFailure: function(err) {
                                                                        //display notif
                                                                        if(widget_context) {
                                                                            var message = RequirementErrorManager.getParseErrorMessage(err,null,editor.lang.rcowidget.serverError);
                                                                            RequirementAlertManager.errorAlert(message);
                                                                    ...
                                                                    onTimeout: function(err) {
                                                                        ...
                                                                        if(widget_context) {
                                                                            var message = RequirementErrorManager.getParseErrorMessage(err,null,editor.lang.rcowidget.serverTimeout);
                                                                            RequirementAlertManager.errorAlert(message);
