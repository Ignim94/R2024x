 ...
 * @quickreview VAI1 ZUD 22 May 2024 : IR-1255571- Enabled creation of wrapper div for footnotes Section element
 * @quickreview UW9 VAI1 07 Mar 2024 : IR-1255571- Enabled creation of wrapper div for footnotes Section element
 * @quickreview UW9 VAI1 07 Mar 2024 : IR-1239016 - Cut markers data needed to be stored locally in editor instance properly for usage in case of paste operation
 * @quickreview UW9 24 Feb 2024 : IR-1239006 -Set draggable property for footnotesmarker widget to false to prevent drag operation
 * @quickreview UW9 VAI1 23 Feb 2024 : IR-1235730 - NLS support for Footnotes
 * @quickreview UW9 VAI1 16 Feb 2024 : IR-1235735 - For last marker deletion as per original code footnotes section along with wrapper is removed as we are not using wrapper - we can remove section directly
 * @quickreview UW9 VAI1 07 Feb 2024 : FUN138526 -  Customized footnotes section
  */
...
(function () {
    ...
    CKEDITOR.plugins.add('footnotes', {
        ...
        init: function (editor) {
            ...
            // Add some CSS tweaks: removed grey color from background
            ...
            editor.on('change', function (evt) {
                ...
                if (editor.skipFootnoteOnChange){
                    editor.skipFootnoteOnChange = false;
                    editor.resetDirty();
                    return
                }
                ...
                if (footnote_section && footnote_section.$.className.indexOf('footnotes') != -1) {
                    $this.update_footnotes_store(editor);
                    return;
                }
            ...
            editor.widgets.add('footnotes', {
                ...
                draggable: false,
                //prevented wrapper creation around footnotes section so as to allow footnotes edition
            ...
            editor.ui.addButton('Footnotes', {
                ...
                label: CKEDITOR.footnotesNls.footnotesTooltip,
        ...
        addFootnote: function (footnote, editor) {
            ...
            if (footnotes === null) {
                ...
                //added horizontal line to distinguish footer section
                container += '<hr style="height:1px;border-width:0;color:gray;background-color:gray;width:70%">';
                // Add header
                if (!editor.config.footnotesDisableHeader) {
        ...
        reorderMarkers: function (editor) {
            ...
            if (contents.find('.footnotes').toArray().length == 0 && isPasted == false) {
            ...
            for (i; i < l; i++) {
                ...
                if (contents.findOne('.footnotes') == null && isPasted == true) {
                    this.addFootnote(this.buildFootnote(footnote_id, footnote_text, data, editor), editor);
                }
            ...
            //As wrapper is not formed around footnotes section no need to use initEditable
        ...
        update_footnotes_store: function (editor) {
            var results = [];
            for (var k in editor.footnotes_tmp)
                results.push([k, editor.footnotes_tmp[k]]);
            if (editor.footnotes_tmp) {
                results.forEach(function (elem) {
                    try {
                        var local_content = editor.editable();
                        editor.footnotes_store[elem[0]] = local_content.findOne('.footnotes [data-footnote-id="' + elem[0] + '"] cite').getHtml();
                    }
                    catch {
                        if (editor.footnotes_store[elem[0]] == undefined) {
                            editor.footnotes_store[elem[0]] = elem[1];
                        }
                    }
                })
            }
