...
<%
	...
	if(object.isKindOf(context, type_MCADAssembly) || object.isKindOf(context, type_MCADComponent) || object.isKindOf(context, type_MCADRepresentation)|| object.isKindOf(context, type_MCADDrawing)) {
        ...
        for(int i=0; i < imageInfoList.size(); i++) {
			...
			String imageid = (String)imageInfo.getCADObjectId();
            if(imageid != null && imageid!="") {
%>          
				// XSSOK
				backImage["<%=i%>"] = new ImageInfo("<%=i%>", "<%=imageInfo.getImageName()%>" , "<%=imageInfo.getImageURL()%>", "<%=imageInfo.getFileExtension()%>", "<%=imageInfo.getCADObjectId()%>", "<%=imageInfo.getCADObjectType()%>", "<%=serverUrl%>");
<%          
			} else {
%>
