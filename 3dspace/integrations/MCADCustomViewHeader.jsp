...
<%
	...
	String localisedDefaultPrefix    = integSessionData.getStringResource("mcadIntegration.Server.FieldName.DefaultTableName");
	
	Context context					= integSessionData.getClonedContext(session);
	String defaultPrefixInEnglish = EnoviaResourceBundle.getProperty(context,"iefStringResource",new Locale("en-US"),"mcadIntegration.Server.FieldName.DefaultTableName");
...
<%
	...
	for(int i=0; i<allTableNames.size(); i++)
	{
		...
		tableListActualValue = MCADUtil.replaceString(tableListActualValue, localisedDefaultPrefix, defaultPrefixInEnglish);
...
%>
        <!--XSSOK-->
        <option value="<%=tableListActualValue%>" <%= isSelected %>><%=tableListDisplayValue%></option>
<%
	}
%>
          		</select>
