...
function cmEditPreProcess (){
	...
	changeDisplayUnit();
        try {
            if (mode_CM == "edit" && displayUnit_CM == "") {
                $('#DisplayUnitId option[value=NONE]').attr('selected','selected');
            }
        } catch { console.log("CM Mode is not defined"); }
	}
...
function displayHideFieldsforEditCopy_Dimension(){
	...
	if(dimension == 'COLORParameter'){
		...
		if(IsCopy == "1"){
			...
			hide('CharacteristicsLowerSpecificationLimit');
			hide('CharacteristicsUpperSpecificationLimit');
			hide('CharacteristicsLowerRoutineReleaseLimit');
			hide('CharacteristicsUpperRoutineReleaseLimit');
		}else{
...
function hide(field) {
	$('#calc_'+field).next('tr').hide();
	$('#calc_'+field).next('tr').next('tr').hide();
}
...
function show(field){
	...
	$('#calc_'+field).next('tr').show();
	$('#calc_'+field).next('tr').next('tr').show();
...
function clearAll(){
	...
	$('#CharacteristicsLowerSpecificationLimit').val("");
	$('#CharacteristicsUpperSpecificationLimit').val("");
	$('#CharacteristicsLowerRoutineReleaseLimit').val("");
	$('#CharacteristicsUpperRoutineReleaseLimit').val("");
}
...
//	$('#DisplayUnitTextId').hide();
	...
	showMultiValue();
	show('CharacteristicsLowerSpecificationLimit');
	show('CharacteristicsUpperSpecificationLimit');
	show('CharacteristicsLowerRoutineReleaseLimit');
	show('CharacteristicsUpperRoutineReleaseLimit');
	enableDUandVT();
...
function isNumericFormat(){
	...
	if((dimension!='StringParameter') &&
		...
		if(! isCharNominalMinMaxNumeric(this.id))
			return false;
...
function isCharNominalMinMaxNumeric(fieldName){		
		...
		if(val != ""){
			...
			var isInRange = isValinRange(fieldName, val);
			if(!isInRange)
				return false;
		}
		return true;
}
function isValinRange(fieldName, value){
	
	var fields = ["CharacteristicsLowerSpecificationLimit", "CharacteristicsLowerRoutineReleaseLimit", "MinimalValue", "NominalValue", "MaximalValue", "CharacteristicsUpperRoutineReleaseLimit", "CharacteristicsUpperSpecificationLimit"];
	var displayFields = ["Lower Specification Limit", "Lower Routine Release Limit", "Lower Target", "Value", "Upper Target", "Upper Routine Release Limit", "Upper Specification Limit"];
	
	var fieldIndex = fields.indexOf(fieldName);
	
	var minValue = $("#"+fields[fieldIndex-1]).val();
	if(minValue != "" && minValue != null){
		//if(parseInt(value) < parseInt(minValue)){
		if(parseFloat(value) < parseFloat(minValue)){
			alert(LowerUpperLimitOutOfRangeAlert);
			return false;
		}
	}
	var maxValue = $("#"+fields[fieldIndex+1]).val();
