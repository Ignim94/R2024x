...
String defaultValNLSNotIntOverLimit = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.AttributeNotIntegerOverLimit","Default Value");
String rangeNLSNotIntOverLimit = DAuUtilities.getSpecializationStringResource(context,"emxDataSpecialization.AttributeNotIntegerOverLimit","One of authorized values");
					...
					var maxLength="<%=attrLength%>"; 
						  if (Type =="Real") {
							...
							if (isDefaultValueFieldNotEmpty() && ! reg_SInteger.test(parseInt(defValue)))
							{
								...
								return false;
							} else if(parseInt(defValue) >= maxIntVal || parseInt(defValue) <= minIntVal) { // Over limit
								alert("<%=defaultValNLSNotIntOverLimit%>");
								return false;
							}
							if(isDefaultValueFieldNotEmpty())
							{
							...
							if(authorizedValues.length !=0){
								for(i=0; i<authorizedValues.length;i++){
									...
									{ 
										...
										return false;
									} else if(authorizedValues[i] >= maxIntVal || authorizedValues[i] <= minIntVal) { // Over limit
										alert("<%=rangeNLSNotIntOverLimit%>");
										return false;
									}
								}
							}
...
var nlsNameNotAlphaNum = "<%=nlsNameNotAlphaNum%>";
var nlsIconNameNotAlphaNum = "<%=nlsIconNameNotAlphaNum%>";
var nlsPackNameNotAlpha = "<%=nlsPackNameNotAlpha%>";
var nlsPackageExist = "<%=nlsPackageExist%>";
var errPrefixLength = "<%=errPrefixLength%>";
		...
		//and check that the Max Length is an Integer
			...
			if (undefined != document.forms['editDataForm'].elements['defaultValue'] && isDefaultValueFieldNotEmpty() ) {
				...
				{	
					alert("<%=defaultValNLSNotInt%>");
					return false;
				} else if(defValue >= maxIntVal || defValue <= minIntVal) { // Over limit
					alert("<%=defaultValNLSNotIntOverLimit%>");
					return false;
				}
				document.forms['editDataForm'].elements['defaultValue'].value = parseInt(defValue);
